...
- **`src/types.ts:1510` dead anchor.** The JSDoc for `triage.criticOverride` (and its `@deprecated v8.58` audit-log successor block) pointed at `.cclaw/lib/runbooks/critic-stage.md > Verdict handling`. `critic-stage.md` is in `src/install.ts > RETIRED_RUNBOOK_FILES` (listed alongside `self-review-gate.md`, `ship-gate.md`, `discovery.md`, `plan-small-medium.md`, `plan-critic-stage.md` — see `install.ts:412` orphan-cleaner) and is removed on upgrade by `removeRetiredLibFiles`. The canonical critic-stage runbook is `.cclaw/lib/runbooks/critic-steps.md`. The JSDoc anchor is the verdict-handling pointer downstream readers follow when reconstructing why a slug carries `criticOverride=true`; a dead anchor breaks the audit trail it's supposed to enable. Mechanical rename only — the JSDoc semantics and the surrounding deprecation note are unchanged.
- **`README.md:120` stale skill count (32 → 34).** README's deeper-docs index claimed "32 auto-trigger skills loaded per stage". The actual count is 34 — `tests/unit/v8106-cleanup.test.ts:75` already pins `expect(AUTO_TRIGGER_SKILLS.length).toBe(34)`, and the build mirror script logs `[copy-skill-md] mirrored 34 skill .md files → dist/content/skills/`. v8.111 added `pre-commitment-predictions` (lifting the count from 32 → 33) and v8.112 added `writing-skills` (33 → 34); the README count row was missed in both PRs. The `v884-not-doing-gate` regression guard (`README.not.toContain("35 skills")`) is preserved — 34 sits below the upward-drift fence.
- **`README.md:122` stale runbook count (23 → 28).** README's deeper-docs index claimed "23 on-demand runbooks loaded by trigger". The actual count in `src/content/runbooks-on-demand.ts > ON_DEMAND_RUNBOOKS` is 28: v8.111's diet pass added `builder-self-review-gate`, `builder-tdd-walkthrough`, `parallel-worktree`, `clarify-protocol`, `plan-md-templates` (the five `.md` files mirrored to `dist/content/runbooks/` by `scripts/copy-skill-md.mjs`), lifting the count from 23 → 28. The README row was missed in v8.111. Companion tripwire `tests/unit/v894-docs-drift-sweep.test.ts > RUNBOOKS_CANONICAL` bumped 23 → 28 in lockstep so both the generic regex sweep (`\b(\d+)\s+runbooks?\b` mismatch against canonical) and the explicit README pin (`expect(README).toMatch(/28 on-demand runbooks/)`) catch future drift.

### Tests
- `tests/unit/v894-docs-drift-sweep.test.ts > RUNBOOKS_CANONICAL` bumped 23 → 28 (the regex tripwire was self-defeating pre-fix: both the canonical constant and the README claim drifted together, so the test continued to pass against the stale README row). The comment block at the top of the file documents v8.111's 5-runbook addition so the next reader understands the bump. Generic regex sweep + explicit README pin both retargeted in lockstep.

### Audited but not shipped (HARDENING / ARCH — defer for separate arc decisions)
- **HARDENING: `src/types.ts > CeremonyMode` JSDoc still describes `strict` mode as `red(AC-N):` / `green(AC-N):` / `refactor(AC-N):` / `test(AC-N):` / `docs(AC-N):`** (`src/types.ts:1217-1220`). v8.110 retargeted the auto-loaded skill bodies (`ac-discipline.md`, `tdd-and-verification.md`, `commit-hygiene.md`, `conversation-language.md`, `plan-authoring.md`) to the v8.63 split (slice work `(SL-N)` + AC verify `verify(AC-N): passing`) but the JSDoc + four other documentation surfaces still teach the pre-v8.63 shape as canonical strict-mode contract. Needs a coordinated rewrite (the antipattern catalog frames A-1 as "TDD phase integrity broken" with `red(AC-N)` examples; flipping it requires a design call on whether A-1 covers both `(SL-N)` and archived `(AC-N)` shapes or splits into A-1a / A-1b).
- **HARDENING: `src/content/antipatterns.ts > A-1`** (`:8-19`) — heavily teaches `red(AC-N) / green(AC-N) / refactor(AC-N)` as the canonical strict-mode contract; doesn't yet describe the v8.63 split. Central document (gets written to `.cclaw/lib/antipatterns.md` at install time; every specialist that detects A-1 cites it).
- **HARDENING: `src/content/stage-playbooks.ts > BUILD_PLAYBOOK`** (`:167-291`) — teaches the pre-v8.63 cycle as the current TDD discipline; gets written to `.cclaw/lib/runbooks/build.md` at install time.
- **HARDENING: `src/content/runbooks-on-demand.ts:260`** — dispatch-envelope prose teaches `red(AC-N) / green(AC-N) / refactor(AC-N) / test(AC-N) / docs(AC-N)` as the canonical strict-mode shape.
- **HARDENING: `src/content/cclaw-rules.ts > ANTIPATTERN_SUMMARIES`** (`:64-70`) — A-1 one-line summary still cites `red(AC-N)` / `green(AC-N)`; would update in lockstep with `antipatterns.ts` above.
- **ARCH: `src/posture-validation.ts` is prompt-referenced but never invoked at runtime.** Same shape as v8.109's deleted dead infra (`path-mutex.ts` / `compound.ts` / `outcome-detection.ts`). `tdd-and-verification.md:246/:248/:253` cites `src/posture-validation.ts:validatePostureTouchSurface(...)` as the canonical predicate the reviewer should run, but the LLM-reviewer doesn't invoke TS. Either delete the helper and inline the predicate body in the skill, or wire a runtime gate that LLMs actually call — needs an identity-call (v8.109 redux).
- **ARCH: `src/assumption-validation.ts` is never invoked at runtime.** Seven exports (`parseValidatesPayload` / `parseAssumptionRows` / `flipAssumptionRows` / `collectValidations` / `unvalidatedKaIds` / `renderUnvalidatedAssumptionsBody` / `replaceUnvalidatedAssumptionsSection`) are imported only by `tests/unit/v885-*` and `v894-*`. The `validates: KA-N` commit-message chain is enforced via prompt contract (`reviewer-axis-assumption-coverage.md` + `builder.ts`), not via runtime helper. Same v8.109 dead-infra shape — defer until the identity call covers both modules.
- **ARCH: `src/slice-topology.ts > topologicalLayers()` is never invoked at runtime.** Specialist prompts cite it as "canonical implementation" (`builder.ts:214` / `:221` / `:251` / `:295`; `slice-discipline.md:29`; `parallel-worktree.md:15`) the builder LLM "reasons about"; the LLM doesn't run TS. Same v8.109 dead-infra shape.
- **ARCH: `src/slice-worktree.ts` has partial runtime invocation.** Only `cleanupSliceWorktreeAsync` is wired (`cancel.ts:14/:64`); `createSliceWorktree`, `mergeSliceWorktree`, `sliceWorktreePath`, `sliceWorktreeBranch` are test-imported only. Specialist prompts cite the helpers as canonical worktree mechanics but the builder uses raw `git worktree` commands. Partial dead-infra — same v8.109 shape.

## 8.112.0 - 2026-05-21

### Cleanup (refactor / chore)
- **Retired 10 flags + 5 namespace shapes (~−12% surface).** Three families of flags rotted into back-doors around immutable triage decisions or duplicates of heuristic / prefix-dispatch paths; one family (`--mode=auto` / `--mode=step`) had been parser-noise since v8.61. The 10 retired flags: `--inline` / `--soft` / `--strict` (back-door around triage's `ceremonyMode` decision — triage heuristic is now the sole source of truth; the decision is stamped once and is immutable for the slug), `--research` (duplicate with the `/cc research <topic>` prefix dispatch), `--light` / `--standard` / `--deep-product` (research-mode depth heuristic infers from topic complexity / lens count / surface scope), `--capture-learnings` (compound gating already runs the 4-condition heuristic: non-trivial D-N + reviewer security-axis flag + ≥3 review iterations + cross-cutting touch — the manual override was double-counting), `--mode=auto` / `--mode=step` (back-compat parser surface for a contract retired in v8.61). The 5 retired namespace shapes (the gsd-pattern router): `/cc-plan` / `/cc-build` / `/cc-review` / `/cc-ship` / `/cc-compound-refresh` + `--enter=` support — `start-command.ts:225` itself read "Non-mandatory — /cc alone covers everything"; the router was inert ceremony. **Kept** (genuine opt-in overrides the heuristic can't infer): `--critic-cross-model`, `--review` (patch lite), `--lens=design` / `--lens=-design`, and every CLI flag from `cli.ts` (CI surface: `--non-interactive`, `--harness=`, etc.).
- **Affected surfaces.** `src/content/start-command.ts` (Detect matrix flag-parsing branches removed; namespace-router section deleted; `#### critic` cross-model paragraph rewritten for the convergence loop; learnings dispatch rewritten as heuristic-only); `src/content/runbooks-on-demand.ts` (`PATCH_MODE` / `RESEARCH_MODE` / `RESEARCH_DEPTH_AND_SELF_REVIEW` / `EXTEND_MODE` / `COMPOUND_REFRESH` / `TRIAGE_GATE` envelopes scrubbed of all retired flag parsing); `src/content/specialist-prompts/triage.ts` (entire `Override flags` section removed; `Modes` table now lists only `heuristic`; anti-rationalization rows rewritten to teach heuristic-only decisions); `src/content/specialist-prompts/{architect,builder,critic,investigator,plan-critic,qa-runner,reviewer}.ts` (cross-references to retired flags rewritten as prose); `src/cli.ts > HELP_NOTES` (Per-flow flags section rewritten — only the three remaining flags survive); `README.md > ## Triage` (heuristic-only framing); `src/types.ts` (`ResearchMode` / `ResearchDepth` / `DEFAULT_RESEARCH_DEPTH` / `RunMode` / `TriageDecision` JSDoc updated); `src/content/artifact-templates.ts > research.md` template (frontmatter comments rewritten); `src/content/core-agents.ts > triage` description (heuristic-only); `src/content/skills/{investigation-discipline,conversation-language,tdd-and-verification,receiving-feedback}.md` (example flag usage rewritten); `src/content/runbooks/patch-mode.md` (removed `--research` / ceremony-flag bypass sections); `src/content/stage-playbooks.ts:145` (back-compat parser-surface note for `--mode=step`).
- **Tripwire.** New `tests/unit/v8112-flag-cleanup.test.ts` (15 tests across 5 describe blocks): the 10 retired flag literals are absent from `start-command.ts` + `runbooks-on-demand.ts` source + the rendered orchestrator body + every on-demand runbook body + the triage specialist prompt; the 5 retired namespace shapes are absent from `start-command.ts` source + rendered body + `runbooks-on-demand.ts`; `README.md` + `cli.ts > HELP_NOTES` are clean of all 15 retired items; the triage heuristic IS the source of truth for `ceremonyMode` (regex check on `triage.ts` + the orchestrator's anchor of immutability); the new `writing-skills` meta-skill exists on disk with the required body sections + registers in `AUTO_TRIGGER_SKILLS`; the critic prompt's `§3.5` documents the convergence-loop contract verbatim. `tests/unit/v858-router-research.test.ts` flipped from presence-asserting to absence-asserting on the retired ceremony / run-mode / research flags + the retired router-flag surface.

### `writing-skills` meta-skill (enhancement)
- **New meta-skill at `src/content/skills/writing-skills.md`** (~220 lines markdown; no infra). Codifies the RED → GREEN → REFACTOR validation cycle adapted for cclaw skills (obra-superpowers namesake; reference: `obra-superpowers/skills/writing-skills/SKILL.md`): a new skill is justified iff it covers ≥3 distinct trigger surfaces / dispatches; the validation cycle requires a deliberate RED baseline (subagent reproduces the failure without the skill) → GREEN (skill compiles, registers, and auto-triggers on the canonical task / file-edit / specialist surface) → REFACTOR (loopholes closed via anti-rationalization patterns + cross-skill cross-references). Body sections: taxonomy (when to use an inline block vs an on-demand runbook vs an auto-trigger skill), mandatory frontmatter schema (`id` / `triggers` / `description` / front-loaded body), the cclaw skill-registration mechanism in `skills.ts`, anti-rationalization patterns specific to skill authoring (the "I'll add a knob" / "this is too small for a skill" / "let me inline it for now" trap), two pointer references to existing canonical models (`pre-commitment-predictions.md` + `anti-slop.md`), and a self-application worked example. Auto-trigger surface: `task:add-skill` + `before:edit src/content/skills/` + `before:edit src/content/skills.ts` — the skill fires when an agent is about to write a new skill. Registered in `src/content/skills.ts > AUTO_TRIGGER_SKILLS` with id `writing-skills`; `tests/unit/v8106-cleanup.test.ts` skill count bumped 33 → 34.

### Santa-loop convergence in critic (enhancement)
- **`src/content/specialist-prompts/critic.ts > §3.5 Cross-model second opinion` rewritten as a 3-round adversarial convergence loop** (reference: `affaan-m-ecc/commands/santa-loop.md` — "two independent models must BOTH return NICE"). Pre-v8.112 behaviour on the `crossModelCritic: true` envelope flag was a one-shot second opinion (Critic A authors `## 3. Adversarial findings` + a cross-model MCP dispatch authors `## Cross-model second opinion`; findings merged once). v8.112 promotes the cross-model pass to a convergence loop on the same trigger set: each round runs Critic A (primary) + Critic B (cross-model via `user-codex` / `user-gemini` / comparable MCP — gstack `/codex` pattern) in parallel, cold and independent (Critic B never sees Critic A's findings to prevent anchoring bias); **both critics must emit `verdict: pass`** before the slug can ship; on any divergence the orchestrator builds a `mode: fix-only` builder envelope from the merged finding set (deduplicated by `<file:line, finding shape>`), the builder lands a single `fix: address critic convergence findings (round N)` commit, and both critics re-dispatch afresh; capped at 3 rounds; non-convergence at round 3 emits `verdict: block-ship` with `note: "cross-model convergence failed"` + a stop-and-report block listing remaining findings (`F-N` from Critic A + `X-F-N` from Critic B) and recommended next steps (`/cc-cancel` to reframe, or `/cc patch <slug> <targeted-fix>` after manual review). The graceful fallback for missing MCP is preserved verbatim — `Cross-model unavailable: skipped.` lands in `critic.md > ## Cross-model second opinion`, the round counter stamps `0`, and the gate degrades to single-critic (Critic A alone). The v8.108 prompt-budget awareness (priority-drop trim list + refuse-and-skip + 16000-char default) is preserved within each round.
- **Convergence counter on `flow-state.json`** — new fields `criticConvergenceRound?: number` (int in `[0, 3]`; `0` = fallback / not applicable; `1` / `2` / `3` = active loop round) and `criticCrossModelVerdict?: CriticVerdict` (Critic B's verdict on the most-recent round); validation added in `src/flow-state.ts`. The orchestrator's post-critic ship gate in `start-command.ts > #### critic` reads BOTH verdicts when `criticConvergenceRound >= 1` (active loop) and only Critic A when `criticConvergenceRound === 0` (fallback / not applicable). Default-path slugs (envelope flag absent on dispatch) bypass the loop entirely; the §3 single-critic verdict remains the sole ship gate. **Behaviour change is fully on the opt-in path** — `--critic-cross-model` user flag (or the heuristic stamps: `securityFlag` + `Reversibility: one-way` D-N + keyword fallback on data-loss / migration / public-API / payment / auth / cryptography) is the only entry; default path is untouched.

### Deferred (not shipped — need separate arc decision)
- **gstack-pattern PreToolUse `careful` hook** (real infra: shell-mode hooks + a `.cclaw/hooks/careful.sh` interpreter; needs an installer hook surface).
- **everyinc-pattern Python skill scripts** (packaging shift: cclaw skills are pure-markdown today; allowing scripts requires a per-skill runtime contract + a sandbox decision).
- **gsd-pattern drift-gate workflow** (new GitHub Actions workflow + a per-slug drift artifact; needs an artifact-retention + sign-off arc).

### Tests
- New tripwire `tests/unit/v8112-flag-cleanup.test.ts` (15 tests, 5 describe blocks). `tests/unit/v8106-cleanup.test.ts` skill count bumped 33 → 34 to cover `writing-skills`. `tests/unit/v858-router-research.test.ts` flipped from presence-asserting to absence-asserting on the retired ceremony / run-mode / research flags + retired router shapes. `tests/unit/v883-token-runbooks.test.ts > V8102_ADDITIVE_CHARS` bumped 0 → 200 to carve out the convergence-loop one-paragraph anchor under `#### critic` (the carve-out itself documents the rationale and confirms it is NOT a re-inline of any lifted runbook body). `tests/unit/v8103-token-diet.test.ts` ceiling relaxed 50_000 → 50_200 with the same carve-out justification documented inline.

## 8.111.0 - 2026-05-21

### Refactor
- **Diet `builder.ts` (~14 KB / ~20% reduction).** Five lifts move long-form prose into dedicated `src/content/runbooks/*.md` files; the in-prompt anchors each carry the load-bearing contract in 3-8 lines + a pointer to the runbook:
  - **A.1 — strict-mode JSON summary block + 5+3 attestation rules** lifted to new `src/content/runbooks/builder-self-review-gate.md`. The orchestrator-gate semantics (`self_review[].verified` across all slice + AC + soft blocks gates reviewer dispatch; failure routes straight to `mode: fix-only` without consuming a reviewer cycle) live in the anchor; the per-slice / per-AC / soft block shapes + the five per-slice rules + the three per-AC rules live in the runbook.
  - **A.2 — patch-mode flow** lifted to existing `src/content/runbooks/patch-mode.md` (extended with a `## Builder protocol` section; the previously-inline `PATCH_MODE` constant in `runbooks-on-demand.ts` was externalised to the .md file and re-wired via `readRunbook()`). Anchor preserves envelope shape, the One-commit-only rule, the no-slice / no-AC / no-flow-state-mutation contract, `Stage: build (patch-mode)` slim-summary shape, and the finalize-step ownership note.
  - **A.3 — bash TDD-cycle worked example** lifted to new `src/content/runbooks/builder-tdd-walkthrough.md`. The in-prompt anchor is 3 lines pointing at the runbook; the full-cycle + REFACTOR-skipped transcripts live in the runbook.
  - **A.4 — topological-layer worktree worked example** lifted to new `src/content/runbooks/parallel-worktree.md`. The in-prompt anchor names the three helpers (`createSliceWorktree` / `mergeSliceWorktree` / `cleanupSliceWorktree`) + the dispatch contract (parent owns the merge; sub-builders never recurse; CI gate per fast-forward); the runbook carries the three-slice worked example.
  - **A.5 — hard rules deduped.** The original 18-rule "Hard rules" list (rules 10 / 11 / 14 / 18 duplicated discipline already taught by the auto-trigger skills `anti-slop.md`, `commit-hygiene.md`, `pre-edit-investigation.md`, `debug-and-browser.md`, `summary-format.md`) consolidated to 6 truly-builder-specific rules + 1 pointer line naming the skill-owned cross-cutting items so the LLM does NOT re-restate them.
- **Diet `architect.ts` (~13 KB / ~23% reduction).** Three lifts:
  - **B.1 — Phase −1 Clarify protocol inner detail** (4-dim scoring math, gap-lens table, challenge-mode stance rotation, hide-render rule, anti-rationalization table) lifted to new `src/content/runbooks/clarify-protocol.md`. The anchor preserves the load-bearing surface inline: the formula `ambiguity = 1 − weighted_sum` with `weighted_sum = goal * 0.4 + constraints * 0.3 + criteria * 0.3 + context * 0.0`, the exit threshold `ambiguity < 0.25`, the stance-rotation rounds 4/5 mention, the v8.105 hide-render rule ("Do NOT render the per-round score table to the user — the math is silent"), the entry condition `clarify_opens = (triage.ambiguityScore >= clarify_threshold) AND (triage.ceremonyMode != "inline")`, the one-question-per-turn discipline, the 5-round cap, the user-signal exit set, and the carry-over rule to plan.md's `## Assumptions (correct me now)` section. The detailed scoring mechanics + 9-row anti-rationalization table live in the runbook.
  - **B.2 — two worked-example `plan.md` transcripts** (soft permission-tooltip excerpt + strict-mode excerpt) lifted to new `src/content/runbooks/plan-md-templates.md`. The in-prompt anchor is 4 lines pointing at the runbook.
  - **B.3 — Phase 10 self-review checklist consolidation.** The original 26-item numbered list (with duplicates of hard rules) collapsed to a categorized 10-row table grouped by Frame / Spec / Slices / AC / Topology, each row naming the check, the modes it applies in (`every` / `strict` / `deep`), and the diagnostic phrase to surface when the check fails.

### Improved (smart-decision sharpenings)
- **`builder.ts:114` — "attempt" definition.** The 2-fix-attempt cap before per-slice `BLOCKED` now defines "an attempt" as one (edit + RED-rerun + GREEN-rerun) cycle that **changes the edit shape from the prior attempt** (different file, different line, different approach). Re-running the same edit twice = 1 attempt for cap purposes. The cap prevents thrashing on a stuck approach, not honest re-verification of flaky tests.
- **`critic.ts:305` — hunting-mode bias check.** Replaces the prior soft "am I inflating severity because I found momentum" framing with a count-based gate: if `block-ship` + `iterate` findings exceed 1.5× the slug's `triage.complexity` baseline (`trivial` = 0; `small-medium` = 2; `large-risky` = 4), the next finding emitted MUST carry an explicit `bias-check: I would have raised this with zero prior findings open` attestation in its description. Findings without the attestation are downgraded one severity. Block-ship findings on data-loss / security / payment are exempt (the NEVER-downgrade rule supersedes).
- **`architect.ts:271` — "not defensible" definition.** The Phase 3 approaches-table dead-option-drop now has a three-filter gate: an approach is "not defensible" iff ANY of (a) it violates an iron-law for the slug's surface (e.g. would require `git push --force` on a shared branch, would add a new runtime dependency on a frozen manifest); (b) the rejection rationale fits in one clause that does not depend on subjective preference (e.g. "needs a database we don't have", not "feels heavyweight"); (c) the architect cannot name two file:line refs where the approach would land. When only one approach passes all three filters, name it + cite the disqualifying clause for each rejected alternative.
- **`investigator.ts:246` — `direct-fix` gate.** The verdict-decision-table row for `direct-fix` now uses a four-condition AND gate: (a) ≤3 file:line refs total across the synthesis; (b) all refs sit under the **same directory** (`src/api/` is one module; `src/api/list.ts` + `src/api/auth.ts` is one module; `src/api/list.ts` + `src/lib/clock.ts` is two modules → `needs-plan`); (c) the fix matches one of the seven mechanical classes from Phase 0.4 fast-path (null guard / typo / missing import / off-by-one / type-coercion / missing-await / missing-return); (d) zero schema / API-shape / auth-boundary touches in the cited file:line(s).
- **`reviewer.ts:259` — `What's done well` band 1-5.** Replaces the prior "at least 1, at most 5" cap with a calibrated band: default emit ONE specific item; 2-3 when the slug touched multiple distinct axes (UI + data + tests) and each axis had a positive non-trivial observation; 4-5 ONLY when the diff is large-risky AND at least 4 axes (out of correctness / readability / architecture / test-quality / edit-discipline / security / perf) carried positive observations — and each item names its axis. Citing the same surface in two items is sycophancy; collapse them.

### Cross-specialist consolidation
- **`pre-commitment-predictions` skill** — new always-on skill at `src/content/skills/pre-commitment-predictions.md` triggered by `specialist:plan-critic` / `specialist:critic` / `specialist:qa-runner` (stages `plan` / `review` / `qa`). Codifies the 3-5 cap rationale, the "before reading the rest" ordering, the outcome enum (`confirmed` / `refuted` / `partial`), the "refuted is information" rule, the "more than 5 is fishing" guidance, the per-specialist mode-flavoured prediction shapes (adversarial expands to 5-7), and a 9-row rationalization-rebuttals table. Replaces three near-identical inline §1 / §1 / §3 blocks in `plan-critic.ts` / `critic.ts` / `qa-runner.ts` with a one-line anchor each pointing at the skill. Registered in `src/content/skills.ts` with id `pre-commitment-predictions`.
- **Confidence-band ladder push into `summary-format.md`** — the canonical three-band ladder (`high` / `medium` / `low`) + the default-hard-gate semantics + the triage exception now live in `src/content/skills/summary-format.md > ## Confidence ladder` (always-on skill loaded on every slim-summary write). Five per-specialist 5-7 line Confidence blocks (architect, builder, reviewer, critic, investigator) collapsed to a one-line anchor + a one-sentence specialist-specific accent. Triage keeps its inline rules (the `Confidence: low NOT a hard gate at triage` rule is the documented exception).

### Tests
- New tripwire `tests/unit/v8111-prompt-diet.test.ts` (38 tests across 7 describe blocks). Asserts: the 6 v8.111-touched runbook .md files exist with the expected sections (5 new + the externalised patch-mode); the 5 new runbooks register in `ON_DEMAND_RUNBOOKS`; the 4 lift sites in `builder.ts` carry the new anchor + drop the long-form content; the 3 lift sites in `architect.ts` carry the new anchor; the 5 smart-decision wordings (C.1-C.5) appear verbatim in their target specialists; the new `pre-commitment-predictions` skill exists + registers + the three inline blocks (`plan-critic.ts`, `critic.ts`, `qa-runner.ts`) collapsed to the one-line anchor; `summary-format.md` carries the canonical confidence-band ladder + the five inline Confidence blocks collapsed (triage keeps its inline rules as the documented exception). Pre-existing tripwires updated for the migration: `v8106-cleanup` skill count bumped 32 → 33; `v8105-polish` / `v878-iterative-clarify` formula assertions verified against the new `goal * 0.4 + ...` notation in the anchor; `v868-per-slice-review` JSON-shape assertions verified against the inline shape sample in the anchor; `v8102-patch-mode` heading + carve-out assertions verified against the rewired patch-mode anchor.

### Note
- Build mirroring: `scripts/copy-skill-md.mjs` now mirrors both `src/content/skills/*.md` AND `src/content/runbooks/*.md` to `dist/content/skills/` + `dist/content/runbooks/` respectively. `package.json > files` includes the new runbook globs. The runbooks loaded at runtime by `readRunbook()` (the v8.111 helper analogous to `readSkill()` in `skills.ts`) resolve under `dist/content/runbooks/` after install; the source-side `.md` files ship for direct inspection.
...
A v8.66 audit pass surfaced two structural gaps that compounded across recent slugs: (1) deep-product research dispatched the standard 5-lens shape with no founder-tier strategic depth — the product lens covered "who benefits / alternatives / market context" but never challenged the premise itself ("right problem? actual outcome? what if we did nothing? inversion?") nor traced second-order strategic consequences (trajectory / identity / adoption / opportunity cost / compounding direction); (2) reviewer's ten-axis check covered correctness, security, performance, and architecture — but had no axis for "is this a usable, coherent, accessible interface?", so AI-slop UI (3-column feature grids, decorative gradients, uniform border-radius, contrast that fails WCAG AA) shipped past review on visual / UX-bearing slugs.

Reference patterns: gstack's `/plan-ceo-review` ("find the 10-star product in the request") + `/plan-design-review` (rate each design dimension 0-10 with explicit "what a 10 looks like" framing) carry the canonical founder-tier + design-tier review shapes. Everyinc-compound's `ce-product-lens-reviewer.agent.md` carries the premise-challenge + strategic-consequences blocks (right problem / actual outcome / what if we did nothing / inversion + trajectory / identity / adoption / opportunity cost / compounding). v8.70 ports these patterns into the cclaw research-product lens (founder mode, gated on `deep-product` depth) and the cclaw reviewer (`design-quality` axis, gated on UI surface).

### What changed

**Deliverable 1 — Founder mode in research-product lens** (`src/content/research-lenses/research-product.ts`).

- New §7 "Premise challenge + Strategic consequences + 10-star reframing" + §8 "Confidence calibration" sections fire alongside §6's existing Thesis + Adjacent-product probes when the dispatch envelope carries `Research depth: deep-product`. Standard / light depths skip founder-mode sections entirely (the gating is a hard rule — partial founder-mode output on `deep-product` is the failure mode this gate was designed to catch).
- Premise challenge walks four questions per research, each emitting a one-paragraph finding when the answer reveals a problem: "right problem?" (could a different framing yield a simpler / more impactful solution?), "actual outcome?" (trace from proposed work to user impact; flag chains of indirection), "what if we did nothing?" (real pain with evidence vs hypothetical need), "inversion: what would make this fail?" (top failure scenario where the plan ships as written and still doesn't achieve its goal).
- Strategic consequences walks five second-order lenses, each emitting a 1-2 sentence note even when clean (the explicit "no concern" stamp keeps the absence honest): Trajectory (toward / away from natural evolution; path dependencies), Identity impact (positioning statement; deliberate vs implicit bet), Adoption dynamics (who gets it easier / who gets it harder), Opportunity cost (concrete competing priority — vague "spending time elsewhere" doesn't qualify), Compounding direction (positive: data / learning / ecosystem; negative: maintenance / complexity tax).
- 10-star reframing fires on every deep-product dispatch except tightly-scoped tactical fixes ("fix this specific bug"). One paragraph describing the maximum-ambition shape of the underlying job-to-be-done; anchors the orchestrator's synthesis pass's "is this worth building at all?" question against a maximum.
- Findings block gains four new sections that render only on deep-product depth: `### Premise challenge`, `### Strategic consequences`, `### 10-star reframing`, plus the existing `### Product thesis` + `### Adjacent product` rows from v8.69 §6 — all five sections are mandatory output on deep-product depth (suppressing them silently is the failure mode the gate was designed to catch).

**Deliverable 2 — Design-quality reviewer axis (gated)** (`src/content/specialist-prompts/reviewer.ts`).

- Reviewer's ten-axis check becomes an **eleven-axis** check. The new gated `design-quality` axis fires when ANY of three conditions hold: (a) the dispatch envelope carries `walkDesignQualityAxis: true` (set by start-command's reviewer dispatch when `triage.designSurface == true` from the v8.70 triage detection); (b) `triage.surfaces` includes any of `"ui"` / `"design"` / `"frontend"` / `"ux"` (architect-written via Phase 1 surface detection); (c) the diff's file list contains at least one file matching `*.tsx` / `*.jsx` / `*.vue` / `*.svelte` / `*.astro` / `*.html` / `*.css` / `*.scss` (fallback heuristic — fires the axis even when triage / architect missed the surface).
- The axis grades each of seven design dimensions `0-10` with an explicit **what a 10 looks like** reference: visual hierarchy (content priority; primary action stands out), type system consistency (3-5 sizes; cascading headings; canonical body / caption / label tiers), color system (constrained palette; semantic state colors; no hex literals embedded), spacing rhythm (4 / 8 / 16 / 24 / 32px scale; related elements cluster), interaction affordances (every button / link affords interaction at rest; loading / empty / error / success / disabled states explicit), accessibility WCAG AA (≥ 4.5:1 contrast; tab order matches reading order; focus rings visible; semantic HTML; ARIA roles), responsive behavior (named breakpoints; touch targets ≥ 44px; no horizontal scroll at 320px).
...
- AI-slop check cross-cuts the seven dimensions: 3-column feature grids, decorative purple/blue gradients, icons in colored circles, uniform border-radius, generic SaaS landing composition (hero + features grid + testimonials + CTA), "modern and clean" as the entire design direction, identical metric cards. ≥2 signals firing → umbrella required finding (axis=design-quality, title=`AI-slop pattern detected`).
- Skip rules: no design surface → emit zero findings; gate fired but diff has zero UI files → skip per-dimension grading with honest-skip note; `triage.downgradeReason == "no-git"` does NOT skip the axis (design quality is independent of git history).
- Per-axis checklist gains a `[design-quality]` block + the slim-summary `What changed` axes counter gains a `dq=N` token (only present when the gate fired; matches the existing `qae=N` gating shape).

**Deliverable 3 — Triage detects design surface** (`src/content/specialist-prompts/triage.ts`).

- New "Design surface detection (v8.70)" section codifies the keyword detection — explicit design-domain (`design` / `redesign` / `UI` / `UX` / `frontend` / `front-end` / `visual` / `layout` / `styling` / `theme` / `Figma`), component / surface vocabulary (`page` / `screen` / `view` / `modal` / `dialog` / `drawer` / `sidebar` / `panel` / `button` / `form` / `landing` / `hero` / `navbar` / `menu` / `tooltip` / `empty state` / `loading state` / `error state`), interaction / accessibility (`accessibility` / `a11y` / `WCAG` / `keyboard nav` / `screen reader` / `responsive` / `mobile` / `breakpoint` / `touch target` / `focus ring` / `aria`), file-pattern hints (`.tsx` / `.jsx` / `.vue` / `.svelte` / `.astro` / `.html` / `.css` / `.scss` / `.tailwind`), and harness hints (`Storybook` / `Tailwind` / `shadcn` / `Radix` / `MUI` / `Chakra` / `Bootstrap`).
- Slim summary gains a new mandatory `Design surface: <true | false>` line. The orchestrator persists the value into `flow-state.json > triage.designSurface` so start-command's reviewer dispatch can stamp `walkDesignQualityAxis: true` on the envelope without re-scanning the prompt at review time.
- The flag is **purely informational** at the routing hop — does not gate ceremonyMode, does not change the triage decision, does not pause for a clarifying question. False-negatives on the flag (missing a UI surface) cost more than false-positives (the reviewer's axis emits zero findings on a gate that fired but found no UI files), so triage favours a generous match.
- New anti-rationalization rows for design-surface scoring: small-keyword-doesn't-need-axis (truth: false-negatives are far more expensive than false-positives), backend-redesign-set-false (truth: don't second-guess the architect's later detection at this hop).

**Deliverable 4 — Orchestrator dispatch envelope** (`src/content/start-command.ts`).

- New one-line bullet under `#### review` declares the design-quality activation contract: orchestrator sets `walkDesignQualityAxis: true` on the reviewer dispatch envelope when `triage.designSurface == true` OR `triage.surfaces` ∩ {`ui`, `design`, `frontend`, `ux`} ≠ ∅. Full rubric (7 dimensions 0-10 + AI-slop check) lives in `reviewer.md > Design-quality axis details`. ~95% of the v8.70 prose lives in `reviewer.ts`.
- The reviewer-stage axis-list line updates to "eleven-axis" (correctness / test-quality / readability / architecture / complexity-budget / security / perf / qa-evidence / edit-discipline / nfr-compliance / design-quality), preserving the v8.62 absorbed-security framing and adding the v8.70 design-quality entry.

**Deliverable 5 — Tests** (`tests/unit/v870-founder-design.test.ts` — new file, 26 tripwire tests; updates to `prompt-budgets.test.ts`, `v822-orchestrator-slim.test.ts`, `v831-path-aware-trimming.test.ts`).

- New `v870-founder-design.test.ts` carries the tripwire suite — research-product founder-mode coverage (premise challenge four-question shape, strategic-consequences five-lens shape, 10-star reframing, deep-product gating preserved on light / standard); reviewer eleven-axis count + design-quality axis declaration + seven dimensions + 0-10 with "what a 10 looks like" framing + below-6 grades become findings + AI-slop check + gating; triage Design surface detection + slim-summary line + canonical keyword classes + responsive / a11y cues + UI file extensions + designSurface persistence prose; orchestrator walkDesignQualityAxis envelope stamping + reviewer-stage axis-list mention; package.json version + CHANGELOG entry. Uses `major === 8 && minor >= 70` semver parse for forward-compat (v8.69's pattern locked in).
- `prompt-budgets.test.ts` raises the reviewer budget `850 → 1100 lines / 85000 → 100000 chars` to absorb the new design-quality axis details (~+65 lines / ~+10k chars over the v8.62 envelope; ~26% line headroom + ~5% char headroom over current ~811 lines / ~95k chars). The 1100-line ceiling matches the v8.70 slug spec's escalation trigger ("stop and report if design-quality axis prose pushes reviewer.ts past 1100 lines").
- `v822-orchestrator-slim.test.ts` + `v831-path-aware-trimming.test.ts` raise the start-command body budgets `81500 → 82000 chars / 548 → 550 lines` (deliberate ~500-char + 2-line bump). The new content is the design-quality envelope-activation bullet under `#### review` and the eleven-axis update on the existing axis-list line. ~95% of the v8.70 prose lives in `reviewer.ts`; the orchestrator body carries inline pointers only. Budget assertions raised together in both files.

### Migration notes

**No breaking changes.** The new `triage.designSurface` field is optional on `TriageDecision`; pre-v8.70 state files lack the field entirely and continue to validate (readers default to `false` on absence, which is identical to the pre-v8.70 behaviour: no design-quality axis activation). The existing `triage.surfaces` field's `ui` / `web` / `cli` / `library` / `api` / `data` / `infra` / `docs` / `other` vocabulary is preserved verbatim — v8.70 adds `design` / `frontend` / `ux` only as **reviewer-side** keywords for the design-quality gate, not as new surface vocabulary the architect emits. Existing slugs whose `triage.surfaces` includes `ui` continue to activate the design-quality axis under condition (b) above.

The reviewer's existing ten axes (correctness / test-quality / readability / architecture / complexity-budget / security / perf / qa-evidence / edit-discipline / nfr-compliance) are preserved verbatim. The eleventh `design-quality` axis is gated; on backend / data / CLI / infra / docs slugs it emits zero findings and notes the structural skip in the iteration block (matching the existing `qa-evidence` / `nfr-compliance` gating shape).

The research-product lens's standard-depth / light-depth output is unchanged. Founder mode is gated entirely on `deep-product` depth; suppressing the founder-mode sections on standard / light is mandatory (the orchestrator's synthesis self-review pass scans for absent sections at the depth tier the user requested).

### Budget bumps documented inline

- **Reviewer prompt char budget** `85000 → 100000` (lines `850 → 1100`). Growth lands ~+65 lines / ~+10k chars from the new "Design-quality axis details" section + the eleven-axis update + the per-axis checklist `[design-quality]` block. The 1100-line ceiling matches the v8.70 slug spec's escalation trigger (the test fires alongside the implementation tripwire). Documented inline in `tests/unit/prompt-budgets.test.ts`.
- **Start-command body char budget** `81500 → 82000` (deliberate ~500-char bump). The new content is the design-quality envelope-activation bullet + the eleven-axis update. ~95% of the v8.70 prose lives in `reviewer.ts`; the orchestrator body carries inline pointers only. Budget assertions raised together in `tests/unit/v822-orchestrator-slim.test.ts` (AC-1 line-count + AC-4 body-alone budget) + `tests/unit/v831-path-aware-trimming.test.ts` (AC-1 line-count + AC-1 char + AC-2 inline-path char).
- **Start-command body line budget** `548 → 550` (deliberate +2-line bump). The new content is one design-quality envelope-activation bullet + the eleven-axis update on the existing axis-list line. Budget assertion raised in `tests/unit/v822-orchestrator-slim.test.ts` (AC-1 line-count guard) + `tests/unit/v831-path-aware-trimming.test.ts` (AC-1 line-count guard).
