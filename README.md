...
| **Review** | 11 axes — 8 base (`correctness`, `readability`, `architecture`, `security`, `perf`, `test-quality`, `complexity-budget`, `edit-discipline`) plus 3 gated (`qa-evidence` when qa-runner ran, `nfr-compliance` when `## Non-functional` is non-empty, `design-quality` (v8.70) when triage detects a UI / design / frontend / UX surface). Append-only findings table, convergence detector, severity-aware ship gate. |
