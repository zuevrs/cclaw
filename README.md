...
| **Review** | 13 axes — 8 base (`correctness`, `readability`, `architecture`, `security`, `perf`, `test-quality`, `complexity-budget`, `edit-discipline`) plus 5 gated (`qa-evidence` when qa-runner ran, `nfr-compliance` when `## Non-functional` is non-empty, `design-quality` (v8.70) when triage detects a UI / design / frontend / UX surface, `scope-drift` (v8.84) when `plan.md > ## Not Doing (and why)` is non-empty — always true post-v8.80 since plan-critic §6.5 blocks ship on empty, `assumption-coverage` (v8.85) when `plan.md > ## Key assumptions to validate` carries ≥1 `KA-N`-shaped bullet). Append-only findings table, convergence detector, severity-aware ship gate. |
