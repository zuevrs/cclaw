...
- `/cc research revise <area>` — re-dispatches the lens(es) covering `<area>` (`engineer` / `product` / `architecture` / `history` / `skeptic` / `synthesis` / `all`), re-runs synthesis + self-review, rewrites `research.md`. Cycles back to `awaiting-user-review` so the user can iterate again.
