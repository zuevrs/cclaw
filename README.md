...
| **Plan template** | 17 sections strict (`Frame`, `Non-functional`, `Approaches`, `Selected Direction`, `Decisions`, `Pre-mortem`, `Assumptions (correct me now)` (v8.67), `Not Doing (and why)` (v8.80 — renamed from `Not Doing`), `Key assumptions to validate` (v8.80), `Plan`, `Spec`, `Plan / Slices`, `Acceptance Criteria (verification)`, `Feasibility stamp`, `Edge cases`, `Topology`, `Traceability block`) — plus two conditionally-appended sections `Plan-design findings` (v8.75; appended by plan-design when its gate fires) and `Plan-devex findings` (v8.82; appended by plan-devex when its gate fires) bringing the strict count to 19 when both gates fire; plus the extend-mode `Extends` section at the top of plan.md on every `/cc extend <slug> <task>` flow. 8 sections soft (`Plan`, `Spec`, `Testable conditions`, `Verification`, `Touch surface`, `Notes`, `Assumptions (correct me now)`, `Not Doing (and why)`). Work-units (slices) are separate from verification (AC). |
...
| **Review** | 12 axes — 8 base (`correctness`, `readability`, `architecture`, `security`, `perf`, `test-quality`, `complexity-budget`, `edit-discipline`) plus 4 gated (`qa-evidence` when qa-runner ran, `nfr-compliance` when `## Non-functional` is non-empty, `design-quality` (v8.70) when triage detects a UI / design / frontend / UX surface, `scope-drift` (v8.84) when `plan.md > ## Not Doing (and why)` is non-empty — always true post-v8.80 since plan-critic §6.5 blocks ship on empty). Append-only findings table, convergence detector, severity-aware ship gate. |
