...
describe("cli (v8.29 TUI-first surface)", () => {
  ...
  it("--help lists the /cc subcommand patterns including the unified /cc <slug> <task> refine (v8.113 — replaces /cc patch + /cc extend)", async () => {
    const ctx = mkContext("/tmp");
    const code = await runCli(["--help"], ctx);
    expect(code).toBe(0);
    ...
    expect(ctx.out.data).toContain("/cc <slug> <task>");
    expect(ctx.out.data).toContain("/cc-cancel");
    // The separate patch/extend keyword subcommands are gone.
    expect(ctx.out.data).not.toContain("/cc patch <slug> <task>");
    expect(ctx.out.data).not.toContain("/cc extend <slug> <task>");
  });
