...
describe("v8.22 orchestrator-slim — token-budget tripwire (body + runbooks)", () => {
  it("AC-4 — body alone is ≤87000 chars (v8.42 lifted to absorb the Hop 4.5 critic pointer; v8.47 lifted by ~300 chars for the design pacing prose; v8.51 lifted ~4k chars for the pre-impl plan-critic sub-step pointer; v8.52 lifted ~7k chars for the qa stage surface-detection + dispatch / verdict-routing pointer; v8.59 lifted ~1k chars for the Detect-hop extend-mode fork pointer + prior-context consumption pointer; v8.63 lifted ~1k chars for the slice / AC separation dispatch envelope clarifiers — Plan / Slices table in the strict plan-stage desc + per-slice TDD + verify(AC-N): passing in the build-stage desc; v8.65 lifted ~9k chars for the multi-lens research orchestrator — 4-phase contract, lens dispatch envelope, lens-output schema, synthesis pass, handoff prompt; the full per-lens contracts live in .cclaw/lib/research-lenses/<lens>.md; v8.66 lifted ~1k chars for the parallel-by-default build-stage prose — topologicalLayers utility pointer + parent / sub-builder dispatch shape + plan-critic safety pre-condition + worktree-parallel coexistence note; v8.71 lifted ~2k chars for the new Phase 3.5 awaiting-user-review pointer + invocation-matrix row + lifecycle vocabulary; v8.72 lifted ~1k chars for the cross-model second opinion pointer under #### critic — trigger set + envelope stamp + graceful-fallback line + gstack /codex reference; v8.74 lifted ~2k chars for the ethos preamble paragraph under Dispatch envelope, the v8.74-promoted cross-model trigger language under #### critic, the Skills-attached `cclaw-ethos` reference doc bullet, and the v8.74 Required ethos read reminder in Always-ask rules)", () => {
    const charCount = renderStartCommand().length;
    expect(
      charCount,
      `start-command body is ${charCount} chars (budget 87000). v8.22 cut ~14% off the body's char count by lifting on-demand runbooks; v8.23 + v8.24 added Hop 1 git-check + the two-pass default paragraph (deliberate ~1k char growth); v8.42 added ~2k chars for the new critic stage pointer; v8.47 added ~300 chars to the large-risky plan section to declare the new two-turn-max pacing (Phase 1 conditional + Phase 7 mandatory + revise-loop semantics; ~95% of the v8.47 content is in design.ts + discovery.md runbook); v8.48 added ~600 chars for the per-AC \`AC verified\` slim-summary line, its semantics paragraph, and the finalize-precondition pointer (the full Per-AC verified gate procedure lives in runbooks/finalize.md); v8.50 lifted ~1500 chars to absorb the knowledge outcome-loop pointers — three brief paragraphs describing the v8.50 follow-up-bug Hop 1 capture path, the outcome-signal down-weight applied to the prior-learnings lookup, and the revert + manual-fix capture paths inside \`runCompoundAndShip\`; v8.51 added ~4k chars for the pre-implementation plan-critic sub-step pointer (one new table row, one paragraph note above the dispatch table, one #### plan-critic body section under #### plan, gating + verdict-routing pointer to runbooks/plan-critic-stage.md); v8.52 added ~7k chars for the qa stage Hop-2 surface-detection block (Surface vocabulary + detection heuristics for each surface), the qa step's dispatch + verdict-routing pointer, and the qa-runner stage-table row (full procedure lives in runbooks/qa-stage.md); v8.59 added ~1k chars for the Detect-hop extend-mode fork pointer + the v8.59 prior-context consumption pointer (full procedures lift to runbooks/extend-mode.md + the per-specialist contracts); v8.65 added ~9k chars for the multi-lens research orchestrator (4-phase contract, lens dispatch envelope, lens-output schema, synthesis pass, handoff prompt); v8.66 added ~1k chars to the build-stage prose for the parallel-by-default contract pointer (topologicalLayers reference + parent / sub-builder dispatch shape + safety pre-condition + worktree-parallel coexistence); v8.71 added ~2k chars for the new Phase 3.5 awaiting-user-review pointer (lifecycle states + three new research sub-commands) + the invocation-matrix row routing /cc research revise|push-back|accept (~95% of v8.71 prose lives in runbooks/research-revision.md); v8.72 added ~1k chars for the cross-model second opinion pointer under #### critic — trigger set + envelope stamp + graceful-fallback line + gstack /codex reference (~95% of v8.72 prose lives in src/content/specialist-prompts/critic.ts §3.5); v8.74 added ~2k chars for the ethos preamble (~1500 chars across the dispatch envelope paragraph, Always-ask rules entry, Skills-attached bullet) + the v8.74-promoted cross-model trigger language under #### critic (~500 chars: Reversibility:one-way primary + Decisions-less keyword fallback). Do not raise this further without a CHANGELOG note.`
    ).toBeLessThanOrEqual(96000);
  });
  ...
  it("AC-4 — combined body + all on-demand runbook bodies stays under a soft 225k-char ceiling (v8.42 lifted to absorb the critic-stage runbook; v8.51 lifted to absorb the plan-critic-stage runbook; v8.52 lifted to absorb the qa-stage runbook + ~7k body chars; v8.59 lifted to absorb the extend-mode runbook + ~1k body chars; v8.65 + v8.66 lifted the combined ceiling to 180k for the multi-lens research orchestrator body + parallel-by-default build-stage prose; v8.68 lifted ceiling to 190k for the structured-status protocol extensions to always-auto-failure-handling.md; v8.69 lifted ceiling to 205k to absorb the new `research-depth-and-self-review.md` runbook (~13k chars); v8.74 lifted ceiling to 225k to absorb the v8.74 ethos preamble (~1.5k body) + the v8.74-promoted cross-model trigger language in critic §3.5 (~500 chars body) + the dispatch-envelope.md runbook expansion absorbing the v8.74 ethos preamble paragraph at the top of the envelope shape (~3k runbook))", () => {
    const combined =
      renderStartCommand().length +
      ON_DEMAND_RUNBOOKS.reduce((acc, r) => acc + r.body.length, 0);
    expect(
      combined,
      `Combined body + on-demand runbooks total ${combined} chars (soft ceiling 225000). The pre-v8.22 body alone was ~50k; the v8.42 critic-stage runbook adds ~7k chars for the dedicated Hop 4.5 dispatch contract; v8.48 added ~3.5k chars total (600 chars body + ~3k chars in finalize.md for the Per-AC verified gate procedure); v8.50 added ~1500 chars to the body for the knowledge-outcome-loop pointer paragraphs (full detection lives in src/outcome-detection.ts + src/compound.ts, NOT in a runbook); v8.51 added ~8k chars total (2k body + ~6k chars in runbooks/plan-critic-stage.md for the gating table, dispatch envelope, verdict routing, iteration cap, flow-state patches, and legacy migration); v8.52 added ~17k chars total (7k body for the qa-stage surface-detection + dispatch / verdict-routing pointer + qa-runner stage-table row, plus ~10k chars in runbooks/qa-stage.md for the gating, dispatch envelope, verdict-routing table, iteration cap, flow-state patches, reviewer cross-check, and legacy migration); v8.59 added ~10k chars total (~1k body for the Detect-hop extend-mode pointer + the v8.59 prior-context consumption pointer, plus ~9k chars in runbooks/extend-mode.md for the argument parsing, parent-validation error sub-cases, seven argument shapes, triage-inheritance precedence rules + worked examples, multi-level chaining policy, and backwards compat); v8.65 lifted the ceiling by ~9k chars in the body for the multi-lens research orchestrator (4-phase contract, lens dispatch envelope, lens-output schema, synthesis pass, handoff prompt); v8.66 lifted the ceiling by ~1k chars in the body for the parallel-by-default build-stage prose (topologicalLayers utility pointer + parent / sub-builder dispatch shape + plan-critic safety pre-condition + worktree-parallel coexistence note); v8.71 lifted the ceiling by ~14k chars total — ~2k body for the Phase 3.5 awaiting-user-review pointer + invocation-matrix row, plus ~12k chars in runbooks/research-revision.md for the revise / push-back / accept procedures, lens-set mapping, fuzzy claim search, failure handling, state-transition table, anti-rationalization; v8.74 added ~5k chars total — ~2k body for the ethos-preamble + v8.74 cross-model trigger language pointers, plus ~3k chars in runbooks/dispatch-envelope.md absorbing the ethos preamble paragraph at the top of the envelope shape (Required ethos read row + rationale paragraph). Expanding past 225k means a block belongs on disk, not inlined and not in a runbook.`
    ).toBeLessThanOrEqual(235000);
  });
});
