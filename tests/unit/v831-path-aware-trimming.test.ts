...
describe("v8.31 path-aware orchestrator — body-only budget", () => {
  it("AC-1 — start-command body stays ≤ 80200 chars (v8.59 raised by 1500 chars for the Detect-hop extend-mode fork pointer + prior-context consumption pointer; v8.63 raised by 1000 chars for the slice / AC separation dispatch envelope clarifiers; v8.65 raised by 9000 chars for the multi-lens research orchestrator; v8.66 raised by 1000 chars for parallel-by-default build-stage prose; v8.68 raised by 200 chars for the structured implementer status pointer in the Always-auto failure handling section — names the four canonical statuses inline; ~95% of the new content lives in `.cclaw/lib/skills/structured-status.md` + the `runbooks/always-auto-failure-handling.md` matrix rows so the body only carries the one-line pointer)", () => {
    expect(renderStartCommand().length).toBeLessThanOrEqual(80200);
  });
