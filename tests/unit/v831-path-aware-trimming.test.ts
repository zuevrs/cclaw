...
describe("v8.31 path-aware orchestrator — per-path envelopes (v8.54: budgets unchanged after merges)", () => {
  ...
  it("AC-2 — inline path budget = body alone, ≤ 85000 chars (v8.59 raised by 1500 chars; v8.63 raised by 1000 chars for slice / AC separation dispatch envelope clarifiers; v8.65 raised by 9000 chars for the multi-lens research orchestrator; v8.66 raised by 1000 chars for parallel-by-default body bump; v8.68 raised by 200 chars for the structured implementer status pointer; v8.69 raised by 1300 chars for the research-depth + synthesis self-review pointers; v8.70 raised by 500 chars for the design-quality envelope pointer; v8.71 raised by 2000 chars for the Phase 3.5 awaiting-user-review pointer + invocation-matrix row + lifecycle vocabulary; v8.72 raised by 1000 chars for the cross-model second opinion pointer under #### critic; see body-only budget above for rationale)", () => {
    expect(bodyBudget([])).toBeLessThanOrEqual(85000);
  });

  it("AC-2 — non-inline path budget = body + 6 runbooks, ≤ 136000 chars (v8.59 raised by 2000 chars: ~1500 chars body + ~500 chars extend-mode pointer; v8.63 raised by 1000 chars for slice / AC separation body bump; v8.65 raised by 9000 chars for the multi-lens research orchestrator's body prose riding into every dispatch; v8.66 raised by 1000 chars for parallel-by-default body bump; v8.71 raised by 2000 chars matching the body bump; v8.72 raised by 1000 chars matching the cross-model second opinion body bump)", () => {
    expect(bodyBudget(NON_INLINE_RUNBOOKS)).toBeLessThanOrEqual(136000);
  });

  it("AC-2 — large-risky path adds parallel-build / cap-reached / adversarial-rerun, ≤ 181000 chars (v8.59 raised by 2000 chars matching the non-inline path bump; v8.63 raised by 1000 chars matching the body bump; v8.65 raised by 9000 chars matching the multi-lens research orchestrator's body bump; v8.66 raised by 1000 chars matching the parallel-by-default body bump; v8.71 raised by 2000 chars matching the body bump; v8.72 raised by 1000 chars matching the cross-model second opinion body bump)", () => {
    const largeRisky = [
      ...NON_INLINE_RUNBOOKS,
      "parallel-build.md",
    ...
    expect(bodyBudget(largeRisky)).toBeLessThanOrEqual(181000);
  });
