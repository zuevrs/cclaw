...
describe("main-menu — actions table", () => {
  it("exposes the seven actions the v8.29 spec required, in display order", () => {
...
describe("main-menu — smart default cursor", () => {
  ...
  it("lands on `sync` when .cclaw/ already exists (re-invocation hint)", () => {
    const state = createMenuState(true);
    expect(selectedAction(state)).toBe("sync");
  });
});
...
describe("main-menu — keyboard reducer", () => {
  ...
  it("vim keys j/k mirror arrow-down/arrow-up", () => {
    ...
    expect(selectedAction(afterJ)).toBe("sync");
    const afterK = applyMenuKey(afterJ, "k").state;
    expect(selectedAction(afterK)).toBe("install");
  });

  it("number keys 1-7 jump directly to the corresponding row", () => {
    let state = createMenuState(true); // starts on `sync` (index 1)
    state = applyMenuKey(state, "5").state;
    expect(selectedAction(state)).toBe("knowledge");
    state = applyMenuKey(state, "7").state;
  ...
  it("Enter confirms the current selection", () => {
    ...
    expect(selectedAction(update.state)).toBe("sync");
  });
...
describe("main-menu — frame rendering", () => {
  it("renders all seven menu rows with their labels", () => {
  ...
  it("first-run frame surfaces the `Install` smart-default hint", () => {
    const frame = renderMenuFrame(createMenuState(false), { useColor: false });
    expect(frame).toContain("no .cclaw/ found — default action is Install");
  });

  it("re-invocation frame surfaces the `Sync` smart-default hint", () => {
    const frame = renderMenuFrame(createMenuState(true), { useColor: false });
    expect(frame).toContain("found existing .cclaw/ — default action is Sync");
  });

  it("places the `>` pointer on the cursor row, not on other rows", () => {
    const state = createMenuState(true); // cursor on `sync` (index 1, label "Sync")
    const frame = renderMenuFrame(state, { useColor: false });
    // Menu rows look like: '  > 2  Sync          ...' and
    //                       '    1  Install       ...'. The smart-default
    // hint line ('default action is Sync') is filtered out by requiring
    // the row prefix shape (two-space gutter + pointer slot + space +
    // index digit + two-space gutter).
    const rowMatcher = (index: number, label: string): RegExp =>
      new RegExp(`^  [> ] ${index}  ${label}`, "u");
    const lines = frame.split("\n");
    const syncRow = lines.find((line) => rowMatcher(2, "Sync").test(line));
    const installRow = lines.find((line) => rowMatcher(1, "Install").test(line));
    expect(syncRow, "Sync row must be present").toBeDefined();
    expect(installRow, "Install row must be present").toBeDefined();
    expect(syncRow!.startsWith("  > ")).toBe(true);
    expect(installRow!.startsWith("    ")).toBe(true);
    expect(installRow!.startsWith("  > ")).toBe(false);
  });

  it("renders the hotkey legend so first-time users discover the controls", () => {
    const frame = renderMenuFrame(createMenuState(false), { useColor: false });
    expect(frame).toContain("Up/Down or k/j to move");
    expect(frame).toContain("1-7 to jump");
