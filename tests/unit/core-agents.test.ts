...
describe("core agents", () => {
  it("ships eight specialists (all on-demand) and two research helpers — v8.62 unified flow dropped `design` (absorbed into `architect`, renamed from `ac-author`) and `security-reviewer` (absorbed into `reviewer`'s `security` axis), renamed `slice-builder` → `builder`, and demoted every specialist (including `architect`) to on-demand activation since v8.61 dropped the main-context dialogue protocol; v8.77 added the read-only debug-branch diagnostic `investigator` between triage and architect; v8.104 merged the v8.75 plan-design + v8.82 plan-devex standalone specialists into the v8.51 plan-critic as `rubricMode: \"design\"` and `rubricMode: \"devex\"` (one specialist, three rubric modes — generic / design / devex)", () => {
  ...
  it("reviewer exposes two modes — `code` (diff/commits) + `text-review` (markdown); integration + release sweeps fold into `code` (ship-path contexts, not standalone modes); adversarial retired into the critic", () => {
    const reviewer = SPECIALIST_AGENTS.find((agent) => agent.id === "reviewer")!;
    expect(reviewer.modes).toEqual(["code", "text-review"]);
    expect(reviewer.modes).not.toContain("adversarial");
    expect(reviewer.modes).not.toContain("integration");
    expect(reviewer.modes).not.toContain("release");
  });
