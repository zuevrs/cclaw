...
describe("parseParallelExecutionPlanWaves ", () => {
...
// ---------------------------------------------------------------------------
// markdown-table-format wave members
// ---------------------------------------------------------------------------

// Realistic 60-line managed block representing W-02..W-05 in table format.
const TABLE_W02_W05_MANAGED_BLOCK = `<!-- parallel-exec-managed-start -->
## Parallel Execution Plan
...
describe("parseTableRowMember ", () => {
  it("extracts S-NN sliceId and verbatim col-2 unitId from a markdown-table row", () => {
  ...
  it("returns null for non-table lines", () => {
    expect(parseTableRowMember("- Path union: 8 files.")).toBeNull();
  });
...
describe("parseParallelExecutionPlanWaves — markdown-table format ", () => {
  it("parses the literal W-02..W-05 managed block (4 waves)", () => {
    const waves = parseParallelExecutionPlanWaves(TABLE_W02_W05_MANAGED_BLOCK);
  ...
  it("accepts a mixed-format managed block (= Members line, = table)", () => {
