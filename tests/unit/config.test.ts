...
describe("config", () => {
  it("keeps explicit empty harness list", async () => {
    ...
    expect(config.agentsMdMode).toBe("full");
  });
  it("falls back to defaults on malformed yaml", async () => {
    ...
    expect(config.agentsMdMode).toBe("minimal");
  });
  ...
  it("rejects invalid agentsMdMode values", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "cclaw-config-invalid-agents-mode-"));
    await fs.mkdir(path.join(root, ".cclaw"), { recursive: true });
    await fs.writeFile(configPath(root), "agentsMdMode: verbose\n", "utf8");
    await expect(readConfig(root)).rejects.toThrow(/unknown agentsMdMode/);
    await expect(readConfig(root)).rejects.toThrow(/Supported values: minimal, full/);
  });
