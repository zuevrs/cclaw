...
| Section | What the linter checks | Source pattern | Source reference |
|---|---|---|---|
| `## Frame the Review Request` | Goal / Approach / Risk areas / Verification done / Open questions | evanflow review frame + superpowers requesting | `evanklem-evanflow/skills/evanflow-review/SKILL.md`, `obra-superpowers/skills/requesting-code-review/SKILL.md` |
...
|---|---|---|
| STOP-per-issue protocol | `stopPerIssueBlock()` | every stage skill, plus structural checks for forcing-questions/failure-modes |
...
| Anti-sycophancy | `antiSycophancyBlock()` | every stage skill; brainstorm + review check explicit acknowledgement |
...
```mermaid
flowchart TD
  B[Brainstorm<br/>Mode + Forcing Q + Premise + APPROACH cards]
