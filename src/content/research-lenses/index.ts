...
export const RESEARCH_LENS_DESCRIPTIONS: Record<ResearchLensId, string> = {
  ...
  "research-design":
    "UI / UX / positioning / affordances lens (v8.76; standard+ depth only, gated on UI / design / frontend / UX topic signals). Walks the seven-dimension design-quality rubric (shared with the v8.104 plan-critic specialist's `rubricMode: \"design\"` body — former v8.75 standalone `plan-design` specialist — and the v8.70 reviewer's design-quality axis) at research framing time — grades each dimension for relevance (load-bearing / relevant / tangential / out-of-scope), surfaces existing patterns to study, anti-patterns to avoid (incl. canonical AI-slop signals), and open design questions for the follow-up architect. Web search first-class via `user-exa` / `user-context7`. Force-toggle via `/cc research --lens=design` / `--lens=-design`."
};
