...
**Underlying mistake.** The TDD cycle was treated as ceremony, not as the contract. The cycle exists so the failing test encodes the AC; skipping or scrambling phases produces an audit trail that nobody can trust. cclaw retired the mechanical pre-commit gate; the contract is now prompt-enforced (builder writes the right prefix) and reviewer-verified ex-post (\`git log\` inspection per posture).

**Correction.** Write a failing test first and commit \`git add tests/<path>.test.ts && git commit -m "red(AC-N): <assertion>"\` (test files only — \`git show <SHA> --stat\` must not include production files). Implement the smallest production change that turns it green; commit \`git commit -m "green(AC-N): <minimal impl>"\`. Either commit a real refactor (\`git commit -m "refactor(AC-N): <one-line>"\`) or land an explicit empty marker (\`git commit --allow-empty -m "refactor(AC-N) skipped: <reason>"\`). The reviewer's posture-aware chain check (using \`src/posture-validation.ts:POSTURE_COMMIT_PREFIXES\`) cites this entry whenever the expected sequence is missing or mis-ordered. For the legacy \`tests-as-deliverable\` posture the expected commit is \`test(AC-N): ...\`; for \`refactor-only\`, \`refactor(AC-N): ...\`; for \`docs-only\`, \`docs(AC-N): ...\` — see the posture-mapping table in \`tdd-and-verification.md\`.
