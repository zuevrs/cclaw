...
- UI / data / infra / docs / config slugs without developer-facing API surface (the devex gate fails — no public interface exists to grade; the `rubricMode: "devex"` dispatch is structurally skipped).
- `triage.ceremonyMode == "inline"` (no plan.md exists; the gate forbids inline dispatch).
- A slug where the diff touches a developer surface but the public contract is unchanged (e.g. an internal refactor that does NOT cross an `export` boundary, a CLI command-handler reorganization that keeps every flag identical) — the `rubricMode: "devex"` dispatch grades the dimensions that ARE exercised and N/A the rest with one-line reasons rather than fishing for findings.
...
Full "what a 10 looks like" anchors live in `src/content/devex-quality-rubric.ts > DEVEX_QUALITY_DIMENSIONS`. The `plan-critic` prompt embeds the table verbatim via the shared renderer on `rubricMode: "devex"` dispatches — do not redefine the rubric in any other place.
...
- **Severity ladder.** `plan-critic` (devex mode): 5/10 → `low`, 4/10 → `medium`, ≤3/10 → `high`.
...
- **NEVER relitigate the architect's API direction.** `plan-critic rubricMode: "devex"` catches **missing commitments** (no error strategy, no migration plan, no measurement hook), NOT alternatives the architect already considered. Alternatives go in Notes (slim summary), never as a finding.
...
| "Measurement is product analytics, not my job." | NO. The devex measurement dimension grades whether the plan instruments **its own** usage so the team learns what shipped (rollout counter, error-rate hook, deprecation log on the old path). A plan with no measurement loops back to "did this fix the developer-experience problem?" with no data — that is a below-6. |
| "I'd design the API differently — let me suggest an alternative as a finding." | Out of scope. `plan-critic rubricMode: "devex"` catches missing commitments in the plan as written; the architect chose the API direction. Surface alternatives as Notes in the slim summary if genuinely important — do NOT file as a finding. |
...
- Runbook (gating + dispatch + verdict routing): `.cclaw/lib/runbooks/critic-steps.md > Plan-devex pass` (covers the `rubricMode: "devex"` dispatch).
