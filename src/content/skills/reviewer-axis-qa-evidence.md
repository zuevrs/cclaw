...
Full rubric, evidence-collection guidance, severity ladder, and anti-rationalizations for the reviewer's `qa-evidence` axis (v8.52+; v8.63 keyed off slice `Surface` for UI gating + AC for evidence rows). Lifted out of `reviewer.ts` in the v8.83 release — the prompt now carries only a 5-line stub pointing here.
