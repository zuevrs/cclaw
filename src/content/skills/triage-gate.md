...
| `["plan", "build", "review", "critic", "ship"]` (small/medium) | one `architect` sub-agent for plan (writes `plan.md` silently in one dispatch); one `builder` for build; one `reviewer` for review (fourteen-axis); critic; ship fan-out | `complexity == "small-medium"` OR `--soft` flag |
