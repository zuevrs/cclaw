...
4. **The traceability block at the end** is reconstructed by the reviewer via `git log --grep="(AC-N):" --oneline` at handoff and at ship time. The plan author writes the AC table and the `commit:` column may be left empty (or pre-filled with the expected commit message shape, e.g. `red(AC-N): ... → green(AC-N): ... → refactor(AC-N): ...`); the builder appends actual SHAs to `build.md` and the reviewer cross-references them against the plan's AC list.
