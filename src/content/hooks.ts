...
export default async function cclawPlugin(ctx) {
  const root = ctx.directory || process.cwd();
  const hooksRoot = join(root, "${RUNTIME_ROOT}", "hooks");
  ...
  const sessionStartPath = join(hooksRoot, "session-start.sh");
  const stopCheckpointPath = join(hooksRoot, "stop-checkpoint.sh");
  const promptGuardPath = join(hooksRoot, "prompt-guard.sh");
  const contextMonitorPath = join(hooksRoot, "context-monitor.sh");
  const observePath = join(hooksRoot, "observe.sh");
  const summarizePath = join(hooksRoot, "summarize-observations.sh");
  ...
  function runHook(scriptPath, args = [], payload = "{}") {
    try {
  ...
  function readTopLearnings() {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!deduped.has(key) || entry.ts > deduped.get(key).ts) deduped.set(key, entry);
      }
      const now = Date.now();
      return [...deduped.values()]
        .map((entry) => {
          const months = entry.source === "user-stated"
            ? 0
            : Math.floor((now - new Date(entry.ts).getTime()) / (30 * 86400000));
          const eff = Math.max(0, entry.confidence - months);
          return { ...entry, effective_confidence: eff };
        })
  ...
  function buildBootstrap() {
    ...
    if (learnings.length > 0) {
      ...
      for (const learning of learnings) {
        parts.push("- " + learning.key + " (conf " + learning.effective_confidence + "): " + learning.insight);
      }
    }
  ...
  return {
    event: async ({ event }) => {
      ...
      if (
        ...
        runHook(sessionStartPath, [], toJson({ event: type }));
      }
      if (type === "session.idle" || type === "session.deleted") {
        runHook(summarizePath, [], toJson({ event: type }));
        runHook(stopCheckpointPath, [], toJson({ event: type }));
      }
      if (
        type === "session.created" ||
        type === "session.updated" ||
    ...
    "tool.execute.before": async (input, output) => {
      ...
      runHook(promptGuardPath, [], payload);
      runHook(observePath, ["pre"], payload);
    },
    "tool.execute.after": async (input, output) => {
      const payload = toJson({
        tool_name: input?.tool || "unknown",
        tool_output: output?.result ?? output?.output ?? input?.result ?? input?.output ?? "",
      });
      runHook(contextMonitorPath, [], payload);
      runHook(observePath, ["post"], payload);
    },
    "experimental.chat.system.transform": async (_input, output) => {
      const bootstrap = buildBootstrap();
      if (!bootstrap || !output || typeof output !== "object") return;
      const current = Array.isArray(output.system)
        ? output.system.filter((value) => typeof value === "string")
        : (typeof output.system === "string" ? [output.system] : []);
      if (current.some((value) => value.includes("cclaw loaded."))) {
        return;
      }
      output.system = [...current, bootstrap];
    },
  };
}
`;
...
- **OpenCode:** lifecycle rehydration + tool-level hooks + stop-time summarization via plugin
...
| OpenCode | \`.opencode/plugins/cclaw-plugin.mjs\` (source: \`${RUNTIME_ROOT}/hooks/opencode-plugin.mjs\`) | event(session.created/updated/resumed/cleared/compacted/idle/deleted), tool.execute.before/after, experimental.chat.system.transform |
