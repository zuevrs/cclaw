...
export const REVIEW: StageSchemaInput = {
  ...
  executionModel: {
    checklist: [
      ...
      "Layer 2b: Security — input validation, auth boundaries, secrets exposure, injection vectors. Mandatory: run a proactive security sweep across diff + touched modules, not just the diff itself, and merge findings into the review army. The Layer 2 security pass is not complete until the sweep records a finding count (0 acceptable) with file:line evidence for every Critical.",
