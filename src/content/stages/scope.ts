...
export const SCOPE: StageSchemaInput = {
  ...
  executionModel: {
    ...
    interactionProtocol: [
      ...
      "**STOP BEFORE ADVANCE.** Mandatory delegation `planner` runs **AFTER user approval of the scope contract**, not before Q&A. Sequence is: Q&A loop -> propose contract -> user approval -> `planner` delegation -> `stage-complete`. If you delegate `planner` before the Q&A loop converges, you violate the elicitation contract and the linter will block stage-complete via `qa_log_below_min`. Three legal fulfillment modes for `planner`: (a) cclaw subagent helper if available; (b) harness-native Task tool — record via `cclaw internal record-delegation --stage=scope --agent=planner --fulfillment-mode=task-tool --evidence-refs=<paths>`; (c) role-switch — announce `## cclaw role-switch: scope/planner (mandatory)`, write the planner output/evidence into the scope artifact, and append a completed delegation row with `fulfillmentMode: \"role-switch\"` plus non-empty `evidenceRefs`. Run `node .cclaw/hooks/stage-complete.mjs scope` from the tool layer (do not paste the command into chat); report only the resulting summary."
    ],
    process: [
