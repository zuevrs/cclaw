...
4. **Decide one of two paths:**
   ...
   **v8.85 — optional \`validates: KA-N\` payload.** When the AC's verification ALSO closes the loop on a \`KA-N\` row in \`plan.md > ## Key assumptions to validate\` (the bet the bullet captures is the SAME bet the AC's evidence proves — e.g. KA-2 reads "search p95 stays under 200ms" and AC-3's verification target is a \`vitest bench\` that asserts the same budget), append a \`validates: KA-N\` line to the commit message body. Multiple KA-N ids are space- or comma-separated on the same line (\`validates: KA-2 KA-4\` or \`validates: KA-2, KA-4\`). Example commit:
   ...
   The post-build flow-state validator (\`src/assumption-validation.ts > parseValidatesPayload\` + \`flipAssumptionRows\`) scans every \`verify(AC-*): passing\` commit in the build range, extracts the \`validates:\` payload, and rewrites the matching KA-N row in \`plan.md\` to \`Status: validated\` with the commit SHA appended (\`Status: validated by <sha>\`). The payload is **truly optional** — most AC do not validate KA rows, and silence is fine. **v8.105 — the payload is optional even for high-stakes rows.** Pre-v8.105 the reviewer's \`assumption-coverage\` axis raised \`required\` (ship-blocking) findings on high-stakes KA-N rows that lacked validating commits; v8.105 caps the axis at \`consider\` (surfaces but never blocks), so a high-stakes row may ship \`unvalidated\` if the build genuinely cannot close the loop. The row still appears in \`ship.md > ## Unvalidated assumptions\` for explicit user acknowledgement. ONLY emit the \`validates:\` line when the AC's evidence actually demonstrates the KA bullet's bet (not "this code uses caching" → KA bullet about cache-hit ratio; the AC's verification target MUST be the same observable the KA bullet's validation method names). False-positive \`validates:\` claims are still surfaced by the reviewer's \`assumption-coverage\` axis (severity=\`consider\` under v8.105 cap-at-consider; pre-v8.105 was \`required\`).
...
## Debug-branch defense-in-depth mode (v8.81; when envelope carries \`defense-in-depth: yes\`)

When your dispatch envelope carries the v8.81 \`defense-in-depth: yes\` field (stamped by the orchestrator from the investigator's slim-summary \`Defense-in-depth: yes\` line; absent OR \`defense-in-depth: no\` means the gate did not fire — ship the root-cause fix alone with no layer additions), you read \`flows/<slug>/investigation.md > ## Defense-in-depth (4 layers)\` and implement **all named (non-n/a) layers as part of the same root-cause fix commit** — NOT as a follow-up commit, NOT as a separate slug. The layers are guards around the fix; they ship together.

The persisted source of truth is \`flow-state.json > builderEnvelope.defenseInDepth\` (string \`"yes"\` / \`"no"\`; absent on pre-v8.81 state files → reads as \`"no"\`). The envelope field and the persisted field always agree on a given dispatch; cross-check before you start (mismatched envelope vs state is an orchestrator bug — stop and surface).
...
2. **GREEN bundles all named layers with the root-cause fix.** The same commit that turns the RED test green also lays down every non-n/a layer at the file:line refs the investigator named in each layer's \`Where:\` line. Do NOT split the layers across separate commits; the v8.81 contract is "defense-in-depth is part of the root-cause fix, not a follow-up" because separating them lets the layers drift out of sync with the fix.
