...
| \`soft\` | **only if risk trigger** | \`gap\` (light) | §1 (predictions, 3 max), §2 (gap analysis), §4 (Criterion check on AC + edge cases + NFR), §6 (realist check), §7 (verdict), §8 (summary). §3 (adversarial findings) **skipped** unless §8 escalation fires. §5 (goal-backward) **collapsed**: one paragraph, not per-criterion. | 5-7k tokens |
...
**Risk trigger (soft gate).** On \`soft\` the orchestrator dispatches the critic ONLY when a risk signal is present: \`triage.securityFlag == true\` OR \`plan.md\` carries any \`D-N\` marked \`Reversibility: one-way\`. Plain \`soft\` slugs (no risk signal) skip the critic entirely — the reviewer is the ship gate. \`strict\` always runs the critic; \`inline\` never does.

If \`ceremonyMode == "inline"\` — or \`ceremonyMode == "soft"\` with no risk trigger — the orchestrator should not have dispatched you. Return a one-line slim summary (\`Stage: critic ✅ complete\`, \`Notes: skipped — <inline | soft, no risk trigger>\`) and stop. Do not write \`critic.md\`.
...
### §3.5. Cross-model second opinion (high-stakes slugs only)

When the dispatch envelope carries \`crossModelCritic: true\`, this section runs a **one-shot cross-model second opinion**: two independent critics walk the same artifacts cold, in parallel, exactly once. **Both critics must emit pass** (\`verdict: pass\`) before the slug can ship; any divergence is an explicit \`block-ship\` (the orchestrator stops-and-reports for user review — there is no automatic fix-and-rerun loop). The contract mirrors affaan-m-ecc's santa-loop ("two independent models must BOTH return NICE") and reuses cclaw's existing cross-model MCP dispatch surface — no new infra. Default-path slugs (envelope flag absent) bypass the second opinion entirely; the §3 single-critic verdict is the sole ship gate.

#### Trigger conditions (envelope flag stamping)

The orchestrator stamps \`crossModelCritic: true\` when ANY of the following fire (the D-N irreversibility signal uses the explicit \`Reversibility: one-way\` field; keyword detection is kept as a fallback for plans with no \`## Decisions\` section):
...
- **any \`D-N\` in \`plan.md > ## Decisions\` is marked \`Reversibility: one-way\` (primary trigger)**. The architect stamps \`Reversibility:\` on every D-N per the PLAN_TEMPLATE D-N row; the orchestrator's pre-dispatch parse looks for the literal \`Reversibility: one-way\` line in any D-N block. plan-critic §A blocks ship on a missing field, so by the time critic runs the field is guaranteed present on every D-N when \`## Decisions\` exists; the trigger is a single substring match on the rendered plan.md, OR
- **keyword fallback** — when \`plan.md\` carries NO \`## Decisions\` section (small strict slugs where Phase 3 was skipped with the "No structural decisions" note; bare bug-fix slugs that authored a soft plan with no D-N machinery), fall back to keyword detection on Blast-radius prose: data loss / data migration / public-API removal / payment / auth / cryptography surface mentioned anywhere in \`plan.md\` body counts as an irreversible signal, OR
...
#### Two-critic shape (both critics run independently)

The second opinion runs **two critics in parallel**:
...
Each critic emits a structured verdict using the cclaw severity vocabulary: \`pass\` / \`iterate\` / \`block-ship\`. Critic B's findings land under \`critic.md > ## Cross-model second opinion\` with their own \`X-F-N\` numbering prefix to keep the audit trail unambiguous:
...
After both critics return, evaluate the verdict gate:

| Critic A | Critic B | Outcome | Next action |
| --- | --- | --- | --- |
| \`pass\` | \`pass\` | **Agreed — NICE** | Proceed to §6 / §7 / §8. Ship gate is open (subject to §6 realist check). |
| \`pass\` | \`iterate\` OR \`block-ship\` | **Diverged — NAUGHTY** | Merge Critic B's findings into §7; emit \`verdict: block-ship\`; orchestrator stops-and-reports. |
| \`iterate\` OR \`block-ship\` | \`pass\` | **Diverged — NAUGHTY** | Merge Critic A's findings into §7; emit \`verdict: block-ship\`; orchestrator stops-and-reports. |
| \`iterate\` OR \`block-ship\` | \`iterate\` OR \`block-ship\` | **Diverged — NAUGHTY** | Merge both critics' findings (dedup by file:line + finding shape) into §7; emit \`verdict: block-ship\`. |

Either critic's \`block-ship\` makes the merged verdict \`block-ship\` (severity wins). The §7 verdict line reads \`Cross-model: Critic A=<pass|iterate|block-ship>, Critic B=<pass|iterate|block-ship>; merged=<agreed|diverged>.\`

#### Non-convergence handling (divergence → block-ship)

A one-shot second opinion does NOT auto-iterate. When the agreement gate fires NAUGHTY, the critic **terminates the dispatch with verdict \`block-ship\`** and \`note: "cross-model second opinion diverged"\`; \`flow-state.json > criticConvergenceRound\` is stamped \`1\` (the single pass ran). The orchestrator does NOT build a fix-only builder envelope and does NOT re-run the critics — divergence is surfaced to the user, not auto-reconciled. The §7 verdict carries:
...
**Recalibration into the verdict.** Cross-model findings carry the same severity vocabulary (\`block-ship\` / \`iterate\` / \`fyi\`) and feed §6 realist check + §7 verdict rollup the same way §3 findings do. A \`block-ship\` \`X-F-N\` blocks ship; an \`iterate\` \`X-F-N\` is captured in learnings.md. The §7 verdict line "Adversarial findings" reports the combined count across both critics' findings (e.g. \`Adversarial findings: 4 total (§3: 3, cross-model: 1); 1 block-ship / 3 iterate / 0 fyi\`). When the cross-model pass returned the \`Cross-model unavailable: skipped\` fallback (no MCP wired), §7 verdict's "Cross-model" rollup line reads \`Cross-model: skipped (MCP unavailable)\` and the dispatch carries \`Confidence: medium\` at minimum (one section of the protocol did not run). When the budget refuse-and-skip fired, the rollup line reads \`Cross-model: skipped (budget overflow)\` instead; same \`Confidence: medium\` floor. When the cross-model second opinion diverged, the rollup line reads \`Cross-model: block-ship (second opinion diverged)\` and the verdict carries both critics' finding sets verbatim per the non-convergence stop-and-report block above.
...
#### §4b. Slice + AC coverage (strict mode; skip in soft mode)

After §4's drift check, also verify the slice/AC machinery actually closed. The plan separates work-units (\`## Plan / Slices\` — SL-N) from verification (\`## Acceptance Criteria\` — AC-N pointing at slices via \`Verifies\`). Both tables must be backed by commits in git log; the critic catches gaps the reviewer's per-finding lens may not have escalated to a meta-finding.
...
4. **Archived-flow shape (single-table detection).** If \`plan.md\` has no \`## Plan / Slices\` section but does have \`## Acceptance Criteria\`, treat the slug as archived-shape and skip checks #1–#3 verbatim. Instead, run the legacy single-table check: every \`AC-N\` MUST have at least one \`(AC-N):\` commit chain in git log per the AC's posture recipe (reviewer's posture-aware checks apply). Coverage gaps here are the same \`G-N\` class=\`ac-coverage\` finding shape, but the recommended fix is "build the AC" rather than "land a verify commit".

Skip §4b entirely in soft mode (single TDD cycle, no per-AC chain to verify). In strict mode with \`legacy-artifacts: true\`, also skip §4b for archived slugs whose plan.md uses the single-AC-table contract — the critic should not gate ship on the absence of a \`## Plan / Slices\` table that never existed.
...
4. **Security flag set.** \`flow-state.json > triage.securityFlag == true\` OR \`plan.md > frontmatter > security_flag == true\`. The reviewer ran the threat-model checklist + sensitive-change rules on its \`security\` axis (which absorbed the former standalone security-reviewer); the critic adds the adversarial stance to the same surface — what failure modes did the threat-model checklist miss?
5. **Near-cap reviewer iterations.** \`flow-state.json > reviewIterations >= 4\` (one short of the 5-iteration cap). The slug needed near-cap iterations to converge; that is a signal that hidden complexity remained even after the reviewer cleared. Concretely: a slug whose reviewer needed to bounce builder back four times is statistically more likely to harbour a gap the reviewer's nine-axis pass could not see.
6. **High prior-learning density.** \`flow-state.json > triage.priorLearnings\` is non-empty AND at least one entry's \`tags\` array contains a known-bad pattern marker (\`A-1\`, \`A-3\`, \`data-loss\`, \`auth-bypass\`, etc.). **Note:** an entry's \`outcome_signal\` of \`reverted\` / \`follow-up-bug\` / \`manual-fix\` is itself a known-bad marker — treat such priors as cautionary precedent (the orchestrator already down-weighted them at lookup; their surface here means the raw similarity was strong enough to clear the down-weight). Entries without \`outcome_signal\` read as \`"unknown"\` (neutral default).
...
Use the budget on the *delta* — gap analysis, pre-commitment, goal-backward, adversarial scenarios. **Do NOT re-walk the reviewer's nine axes.** Read the Findings as already-walked context and spend your budget on what the reviewer's structural framing cannot see.
...
- **Do not re-walk the reviewer's nine axes.** Read the ledger as already-walked context. Re-walking duplicates work and burns budget on already-surfaced findings.
- **Do not raise findings on the security axis using reviewer vocabulary.** Security findings are the reviewer's \`security\` axis surface (which absorbed the dedicated security-reviewer specialist). You may cite a security gap (e.g. "the auth path's edge case is uncovered") but as a \`G-N\` in §2 (class=\`criterion-coverage\`), not as a security-axis finding.
