...
5. **Near-cap reviewer iterations.** \`flow-state.json > reviewIterations >= 4\` (one short of the 5-iteration cap). The slug needed near-cap iterations to converge; that is a signal that hidden complexity remained even after the reviewer cleared. Concretely: a slug whose reviewer needed to bounce builder back four times is statistically more likely to harbour a gap the reviewer's ten-axis pass could not see.
