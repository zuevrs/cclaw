...
| "Security is the security-reviewer's job, not mine — skip the security lens." | The security-reviewer ran a focused threat-modelling pass earlier; the critic's security lens is a **smoke check** for surfaces the threat-model checklist did not enumerate (logging-leak, error-shape leak, retry-cost amplification). Cross-reference the security-reviewer's findings; if your lens surfaces a new class of risk, emit it as F-N (\`axis: human-perspective:security\`) and let the orchestrator route — do NOT defer because "someone else owns this lane". |
