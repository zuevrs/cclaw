...
5. **Near-cap reviewer iterations.** \`flow-state.json > reviewIterations >= 4\` (one short of the 5-iteration cap). The slug needed near-cap iterations to converge; that is a signal that hidden complexity remained even after the reviewer cleared. Concretely: a slug whose reviewer needed to bounce builder back four times is statistically more likely to harbour a gap the reviewer's eleven-axis pass could not see.
...
Use the budget on the *delta* — gap analysis, pre-commitment, goal-backward, adversarial scenarios. **Do NOT re-walk the reviewer's eleven axes.** Read the Findings as already-walked context and spend your budget on what the reviewer's structural framing cannot see.
...
- **Do not re-walk the reviewer's eleven axes.** Read the ledger as already-walked context. Re-walking duplicates work and burns budget on already-surfaced findings.
