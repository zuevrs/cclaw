...
- the active flow's \`triage\` (\`ceremonyMode\`, \`complexity\`, \`assumptions\`) — read from \`flow-state.json\`;
- \`flows/<slug>/plan.md\` (Frame, NFR, AC table, Decisions, Edge cases, Pre-mortem if present, Not Doing, **Prior lessons applied**) — the source-of-truth of *what was promised* AND the **live** prior-lessons recall source (architect-folded via \`learnings-research\`; the deprecated \`triage.priorLearnings\` flow-state field is no longer populated for new flows);
...
6. **High prior-lesson density.** \`plan.md > ## Prior lessons applied\` carries ≥1 lesson whose quote names a known-bad pattern marker (\`A-1\`, \`A-3\`, \`data-loss\`, \`auth-bypass\`, etc.) OR a cautionary outcome (\`reverted\` / \`follow-up-bug\` / \`manual-fix\`). This is the live recall source (architect-folded via \`learnings-research\`, already outcome-weighted); a cautionary precedent that overlaps this slug's surface is exactly the gap the reviewer's nine-axis pass can miss. (The deprecated \`triage.priorLearnings\` field is no longer populated for new flows; read it only on resume of a legacy state file.) **When a recalled prior is the load-bearing reason for a §2 finding, list the slug in your slim summary's \`Recalled-priors: <slug>[,<slug>]\` line** so the orchestrator stamps its usage (\`recall_count\`) at compound time.
