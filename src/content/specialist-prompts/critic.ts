...
**Recalibration into the verdict.** Cross-model findings carry the same severity vocabulary (\`block-ship\` / \`iterate\` / \`fyi\`) and feed §6 realist check + §7 verdict rollup the same way §3 findings do. A \`block-ship\` \`X-F-N\` blocks ship; an \`iterate\` \`X-F-N\` is captured in learnings.md. The §7 verdict line "Adversarial findings" reports the combined count across the round's converged state (e.g. \`Adversarial findings: 4 total (§3: 3, cross-model: 1); 1 block-ship / 3 iterate / 0 fyi\`). When the cross-model pass returned the \`Cross-model unavailable: skipped\` fallback (no MCP wired), §7 verdict's "Cross-model" rollup line reads \`Cross-model: skipped (MCP unavailable)\` and the dispatch carries \`Confidence: medium\` at minimum (one section of the protocol did not run). When the budget refuse-and-skip fired, the rollup line reads \`Cross-model: skipped (budget overflow)\` instead; same \`Confidence: medium\` floor. When the convergence loop terminated at the round-3 cap without convergence, the rollup line reads \`Cross-model: block-ship (convergence failed after 3 rounds)\` and the verdict carries the unconverged finding sets verbatim per the round-cap stop-and-report block above.
