...
2. **Slice quality (per SL-N row).** For each slice, verify three things:
   ...
   - **\`independent\` flag accuracy (v8.64 — parallel-by-default safety gate; tightened to zero-file-overlap in v8.73)** — a slice with \`independent: true\` MUST have an empty \`dependsOn\` AND **zero file overlap** with any other slice's \`Surface\` column. **Zero-file-overlap is the only acceptable proof of independence**: compute the literal set-intersection of every pair of \`Surface\` arrays in the slice table, and any non-empty intersection involving an \`independent: true\` slice is a hard failure — the worktree-isolated dispatch (v8.73) materialises a sibling git worktree per independent slice and fast-forward merges back, but two sub-builders authoring different commits against the same file in different worktrees still produce a fast-forward merge conflict the builder routes through the v8.68 \`BLOCKED\` status protocol. **Severity follows the failure mode**: (a) \`independent: true\` with a non-empty \`dependsOn\` declared is \`iterate\` (the flag is misleading but the explicit \`dependsOn\` will force sequential dispatch anyway — class=\`independence-flag-stale\`); (b) **\`independent: true\` with any file-level overlap against any other slice's \`Surface\` is \`block-ship\` (class=\`independence-mismatch\`)** — the upstream gate exists precisely so the builder never has to materialise worktrees only to merge-conflict them; plan-critic blocks-ship the independence claim that fails zero-file-overlap. The architect must either narrow \`Surface\` (true independence — disjoint file sets) OR add a \`dependsOn\` edge so the second slice schedules in a later topological layer (sequential ordering inside a single worktree). Cite the overlapping file(s) verbatim in the finding body so the architect's revision is one edit. The zero-file-overlap requirement is **literal** — partial-path overlap (e.g. \`src/lib/\` in slice A and \`src/lib/foo.ts\` in slice B) counts as overlap and triggers the same \`block-ship\` finding; the architect must declare files (or non-overlapping subdirectories) at the granularity the reviewer's edit-discipline axis enforces ex-post.
...
- **Bullet without \`KA-N\` id** (v8.85; e.g. \`- **search p95 stays under 200ms**\` with no leading \`KA-N\` bold token) — emit an \`iterate\` finding (class=\`key-assumptions-no-id\`). The id is mandatory so the builder's \`validates: KA-N\` commit payload, the reviewer's \`assumption-coverage\` axis, and the ship template's \`## Unvalidated assumptions\` section can cross-reference rows by id; a bullet without an id silently disables the v8.85 closure loop.
...
§6.5's findings ride the same plan-critic.md findings table; class names (\`missing-not-doing\` / \`empty-not-doing\` / \`not-doing-no-rationale\` / \`missing-key-assumptions\` / \`empty-key-assumptions\` / \`key-assumptions-no-method\` / \`key-assumptions-no-status\` / \`key-assumptions-bad-status\` / \`key-assumptions-no-id\`) make the bets-and-exclusions findings easy to grep for in fix-only rounds. Section header in plan-critic.md is literally \`## §6.5. Bets and exclusions audit\`.
