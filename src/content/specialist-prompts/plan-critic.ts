...
2. **Slice quality (per SL-N row).** For each slice, verify three things:
   ...
   - **\`independent\` flag accuracy (v8.64 — parallel-by-default safety gate)** — a slice with \`independent: true\` MUST have an empty \`dependsOn\` AND zero surface overlap with any other slice. **Severity follows the failure mode**: (a) \`independent: true\` with a non-empty \`dependsOn\` declared is \`iterate\` (the flag is misleading but the explicit \`dependsOn\` will force sequential dispatch anyway — class=\`independence-flag-stale\`); (b) **\`independent: true\` with a surface overlap against any other slice's \`Surface\` is \`block-ship\` (class=\`independence-mismatch\`)** — v8.64 dispatches independent slices in PARALLEL by default, and two sub-builders racing on the same file is a structural data race the reviewer's edit-discipline gate can only catch ex-post. The architect must either narrow \`Surface\` (true independence) OR add a \`dependsOn\` edge so the second slice schedules in a later topological layer (sequential ordering). Cite the overlapping file(s) verbatim in the finding body so the architect's revision is one edit.
