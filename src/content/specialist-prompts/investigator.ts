...
Name the **one** axis (from the reviewer's 11-axis: \`code-quality\` / \`tests\` / \`risk\` / \`acceptance\` / \`coupling\` / \`security\` / \`performance\` / \`error-discipline\` / \`qa-evidence\` / \`design-quality\` / \`<11th>\`) that would have caught the bug class, AND the specific finding text the axis should have produced (one sentence). Examples:
...
- "\`error-discipline\` — \`unawaited-promise\`: the \`saveDraft\` call at \`editor.tsx:118\` returns a Promise but isn't awaited; if it rejects, the symptom is silent data loss. The error-discipline axis's unhandled-rejection scan should have caught it."

**Prevent-recurrence: what review check should be added?**
- **Reviewer axis check** (one bullet): the **one specific check** that should be added to the named axis to catch this class of bug going forward. Frame as "the <axis> axis MUST scan for <pattern> when <gate>", e.g. "the security axis MUST scan for unwrapped admin routes when triage.surfaces includes \`api\`", or "the error-discipline axis MUST scan for unawaited Promise-returning calls when the function name contains \`save\` / \`write\` / \`commit\` / \`flush\` / \`persist\`".
...
- The "What review axis would have caught it?" block names **exactly one axis + one finding**. When multiple axes plausibly apply, pick the one closest to the root-cause mechanism (a missing null guard is \`error-discipline\` even when the symptom is a security exposure — root cause first, blast-radius second).
