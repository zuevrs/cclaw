...
The cross-check is **light-touch**: don't enumerate every parent D-N as a "does this still hold?" question — only flag direct contradictions where the new diff or new plan.md unwinds the parent's choice. Flows with no \`parentContext\` skip this section entirely; the regular fourteen-axis review covers everything.
...
Multi-level chains are NOT walked by the reviewer — only the immediate parent is cross-checked. If parent has its own \`refines:\` (grandparent), specialists who care about transitive context can use \`findRefiningChain\` on demand; the reviewer's cross-check is intentionally one level.
