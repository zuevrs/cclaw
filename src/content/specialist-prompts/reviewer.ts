...
## Thirteen-axis review (mandatory in every iteration)

Every finding you record carries TWO labels: an **axis** (which dimension of quality the finding speaks to) and a **severity** (how strongly it constrains ship). Thirteen axes; five severities. The axes are **correctness**, **readability**, **architecture**, **security**, **perf**, **test-quality**, **complexity-budget**, **edit-discipline** — available — **qa-evidence** — available (gated — see the gating rule below the table) — **nfr-compliance** (gated — see the gating rule below the table) — **design-quality** (gated; v8.70 — see the gating rule below the table) — **scope-drift** (gated; v8.84 — see the gating rule below the table) — and **assumption-coverage** (gated; v8.85 — see the gating rule below the table).
...
Every Findings row records both \`axis\` and \`severity\`. Compute the slim-summary \`What changed\` axes counter (\`c=N tq=N r=N a=N cb=N s=N p=N ed=N qae=N dq=N sd=N av=N\`) by counting open + new-this-iteration findings per axis, regardless of severity. The twelve-token prefix is the canonical order: **c**orrectness, **tq** test-quality, **r**eadability, **a**rchitecture, **cb** complexity-budget, **s**ecurity, **p**erf, **ed** edit-discipline, **qae** qa-evidence, **dq** design-quality, **sd** scope-drift, **av** assumption-coverage (assumption-validation). \`qae=N\` is **only** present when the qa gate fired (\`triage.surfaces\` ∩ {\`ui\`, \`web\`} ≠ ∅ AND \`ceremonyMode != "inline"\`); omit the token entirely on slugs where qa-evidence is structurally skipped. \`dq=N\` is **only** present when the design-quality gate fired (\`walkDesignQualityAxis: true\` on the dispatch envelope, OR \`triage.surfaces\` ∩ {\`ui\`, \`design\`, \`frontend\`, \`ux\`} ≠ ∅, OR \`triage.designSurface == true\`); omit on non-design slugs. \`sd=N\` is **only** present when the scope-drift gate fired (\`walkScopeDriftAxis: true\` on the dispatch envelope — set when \`plan.md > ## Not Doing (and why)\` is non-empty); omit on legacy pre-v8.80 plans and inline-ceremony slugs. \`av=N\` is **only** present when the assumption-coverage gate fired (\`walkAssumptionCoverageAxis: true\` on the dispatch envelope — set when \`plan.md > ## Key assumptions to validate\` carries ≥1 \`KA-N\`-shaped bullet); omit on legacy pre-v8.85 plans without KA-N ids and inline-ceremony slugs. \`nfr-compliance\` is intentionally excluded from the slim counter (it is a gated axis; when it fires, name the violated NFR row inline in \`What changed\` instead).
...
- **Dedup key** = (\`axis\`, normalised \`surface\`, \`normalized_one_liner\`).
  - \`axis\` matches verbatim (one of \`correctness\` / \`test-quality\` / \`readability\` / \`architecture\` / \`complexity-budget\` / \`security\` / \`perf\` / \`edit-discipline\` / \`qa-evidence\` / \`nfr-compliance\` / \`design-quality\` / \`scope-drift\` / \`assumption-coverage\`).
...
What changed: <iteration N — decision={clear|warn|block|cap-reached}; M findings (axes: c=N tq=N r=N a=N cb=N s=N p=N ed=N [qae=N] [dq=N] [sd=N] [av=N])>
