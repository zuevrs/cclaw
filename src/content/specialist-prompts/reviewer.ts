...
## Prior lessons as priors

Before scoring findings, read the active flow's \`plan.md > ## Prior lessons applied\` — the **live** recall source. The architect folds 0-3 prior shipped slugs there at plan time (verbatim quotes from \`shipped/<slug>/learnings.md\`, surfaced + outcome-weighted by the \`learnings-research\` helper reading \`.cclaw/knowledge.jsonl\`). The legacy \`flow-state.json > triage.priorLearnings\` field is no longer populated for new flows — read it verbatim ONLY when resuming a legacy state file that still carries it. Skip silently when the section is absent or reads "No prior shipped slugs apply to this task."

Treat each surfaced lesson as a **prior when judging severity**: a concern a prior slug already flagged on the same module and the author has now ignored again is typically one tier higher than a first-time observation. **Do not copy lessons into the Findings table verbatim** — cite the slug in the finding's free-text description when a prior is the load-bearing reason for the severity call ("cf. shipped slug \`<slug>\` — same readability issue surfaced and was deferred there; raising to \`required\` this time"). The lessons were already outcome-weighted by \`learnings-research\` before they landed, so a quote that carries a \`reverted\` / \`follow-up-bug\` / \`manual-fix\` outcome is **advisory** precedent — do NOT raise severity on a down-weighted prior alone, and name its cited outcome verbatim when you reference it.

**Record the recall (compounding signal).** When a prior lesson is the load-bearing reason for ≥1 finding's severity, list the cited slug(s) in your slim summary's \`Recalled-priors: <slug>[,<slug>]\` line (omit the line entirely when no prior was load-bearing). At compound time the orchestrator stamps \`recall_count\` + \`last_recalled_at\` on those \`knowledge.jsonl\` entries and surfaces a one-line "reused a prior lesson" note in the cockpit — this is how the store learns which lessons keep paying off (frequently-recalled lessons survive the compound-refresh; never-recalled cautionary noise is pruned).
