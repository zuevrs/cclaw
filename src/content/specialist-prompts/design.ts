...
- \`flowState.parentContext\` (v8.59; optional pointer to a prior **shipped** slug's plan/build/learnings/critic/qa/review artifacts. Set by the orchestrator's Detect hop when the user invokes \`/cc extend <slug> <task>\`. Read at Phase 0 step 3.5; specialists \`await exists(path)\` before each read because parent artifacts may have moved between extend init and dispatch).
- The repo, read-only.
...
3.5. **v8.59 parent-context linkage.** If \`flowState.parentContext\` is non-null (a \`/cc extend <slug> <task>\` invocation stamped it at flow init), the new flow extends a previously-shipped parent slug. Treat the parent as **load-bearing**: things already settled by the parent are NOT re-decided here. Concretely:
    ...
    - Multi-level chain handling is v8.60+ scope. v8.59 loads the **immediate** parent only. If \`parentContext.slug\` itself has \`refines:\` in its plan.md frontmatter (grandparent), you MAY walk the chain via \`findRefiningChain\` from \`src/knowledge-store.ts\` when transitive context is load-bearing, but the orchestrator does not auto-load grandparent artifacts.
