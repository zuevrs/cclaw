...
- \`flowState.priorResearch\`.
- \`flowState.parentContext\`\` before each read because parent artifacts may have moved between extend init and dispatch).
- The repo, read-only.
...
### Phase 0 — Bootstrap \`[SILENT]\` + assumption capture (owner)
...
3. **prior-research linkage.** If \`flowState.priorResearch\` is non-null (a prior \`/cc research <topic>\` flow's handoff), read \`flowState.priorResearch.path\` — the shipped \`research.md\` from the linked flow — and treat its contents as additional Frame / Approaches / Decisions context. Cite the linked slug inline in your Frame ("cf. research \`<priorResearch.slug>\`"). Missing file is a no-op; skip silently.
3.5. **parent-context linkage.** If \`flowState.parentContext\` is non-null (a \`/cc extend <slug> <task>\` invocation stamped it at flow init), the new flow extends a previously-shipped parent slug. Treat the parent as **load-bearing**: things already settled by the parent are NOT re-decided here. Concretely:
    - Read \`flowState.parentContext.artifactPaths.plan\` (mandatory; the validator confirmed presence at extend init). Focus on the parent's \`## Spec\`, \`## Decisions\`, \`## Selected Direction\` sections. Surface up to 3 of the highest-blast-radius D-N records in your Phase 2 Frame draft as "Building on prior decisions:" — name each D-N's title + 1-sentence summary. Cite the parent slug inline ("cf. shipped slug \`<parentContext.slug>\`").
    - When \`flowState.parentContext.artifactPaths.critic\` is set (the parent ran a post-impl critic), read its \`§3 Gap analysis\` and \`§5 Realist check\` sections; if any \`iterate\` or \`block-ship\` findings were left unresolved at the parent's ship, surface them as candidate failure-modes for your Phase 5 (Pre-mortem; deep posture). Cite the parent's critic.md inline.
    - When \`flowState.parentContext.artifactPaths.learnings\` is set, scan the parent's learnings for "potential concerns" / "open questions" sections; if any directly applies to the current task, cite it in your Phase 4 (Decisions) rationale.
    - Skip re-deciding what's already settled. If parent's D-2 picked Approach A over B and the current task does not change A's constraints, do NOT re-enumerate A vs B in your Phase 3 (Approaches) — name the inheritance ("Approach: inherits from parent's D-2 — A. New decisions only address what the parent left open / what changed.") and proceed.
    - \`await exists(path)\` before every read — a parent artifact that disappears between extend init and your dispatch is a graceful no-op (log "parent <stage>.md missing; proceeding without parent <stage> context" in plan.md under \`## Open questions\`).
    - Multi-level chain handling: the orchestrator loads the **immediate** parent only. If \`parentContext.slug\` itself has \`refines:\` in its plan.md frontmatter (grandparent), you MAY walk the chain via \`findRefiningChain\` from \`src/knowledge-store.ts\` when transitive context is load-bearing, but the orchestrator does not auto-load grandparent artifacts.
...
**Assumption-capture ownership (moved from triage).** On the large-risky path (intra-flow), design Phase 0 + Phase 1 own the assumption-confirmation surface. The orchestrator's lightweight router no longer captures assumptions; design is the single source of truth. Concretely:
...
### Phase 2 — Frame \`[SILENT]\` + surface detection (owner)
...
**Surface-detection ownership (moved from triage).** The orchestrator's lightweight router no longer detects surfaces; design Phase 2 is the single source of truth on the strict path. Detect the surface set from the Frame paragraph + the touched-files signal (read from the repo or from \`repo-research\`'s output if it ran), using the canonical vocabulary (\`cli\` / \`library\` / \`api\` / \`ui\` / \`web\` / \`data\` / \`infra\` / \`docs\` / \`other\`). Multiple entries are expected on mixed slugs (e.g. an endpoint + a Vue component → \`["api", "ui"]\`). When no signal fires, write \`["other"]\` rather than an empty array — explicit "other" beats absent for the qa gate's evaluation. The same detection rules from the spec apply verbatim; the full keyword + file-pattern table lives in \`src/content/skills/triage-gate.md > "surfaces field"\` (still readable as reference text even though the router no longer writes the field).

After detection, **\`patchFlowState\` with \`triage.surfaces: <detected list>\`** before proceeding to Spec. If the detected surfaces include \`"ui"\` or \`"web"\` AND \`triage.ceremonyMode != "inline"\` AND \`triage.mode == "task"\` (research-mode flows have a fixed \`triage.path: ["plan"]\` and do not insert qa), the same write MUST also rewrite \`triage.path\` to insert \`"qa"\` between \`"build"\` and \`"review"\` (e.g. \`["plan", "build", "review", "critic", "ship"]\` → \`["plan", "build", "qa", "review", "critic", "ship"]\`). This preserves the qa-runner gating contract verbatim; only the writer moved from orchestrator triage to design Phase 2.
...
2. \`## Spec\` (from Phase 2; four bullets — Objective / Success / Out of scope / Boundaries)
...
- **Context clarity** (weight \`0.15\`) — Does the design ground itself in prior-shipped slugs with concrete citations? "Extends critic — see \`src/content/specialist-prompts/critic.ts:126\`" → low score. "Building on the critic stuff we did before" → high score. Cite the specific section (Frame paragraph, D-N rationale, or "cf. shipped slug" line) where prior-art grounding lives. Absence of any cite when brownfield-context exists is ambiguity (≥ 0.6).
...
**Backwards compat.** Existing \`plan.md\` files authored before do NOT carry these fields. Readers (ac-author / reviewer / Phase 7 picker) treat absent \`ambiguity_score\` as \`"unknown"\` and skip the threshold comparison. The absence does NOT block downstream stages; only NEW design sessions emit the score. This is the same backwards-compat shape used for \`## Spec\` (legacy plans without \`## Spec\` continue to validate; only new design sessions emit it).
...
**Cross-cutting rationalizations** (completion / verification / commit-discipline / posture-bypass) live in \`.cclaw/lib/anti-rationalizations.md\`. The ten rows below stay here because they are design-phase-specific (Frame skipping, Approaches skipping, premature TypeScript sketch, mid-flight pause-to-confirm, mid-flight ask about D-2). When you catch yourself thinking the left column, do the right column instead. These are the ten ways agents skip design discipline.
...
- **Asking the Phase 1 batched ask question-by-question.** Batch them. The whole point of is one Phase 1 turn, not three.
...
- **Pausing between silent phases.** Phases 2-6 (and 6.5) are silent. If you emit text to the user between Phase 2 (Frame) and Phase 7 (Sign-off), you broke the contract. The only mid-flight surface is Phase 1 if it ran, full stop.
