...
\`Recommended next\` enum is canonical and matches the values reviewer / security-reviewer / ac-author / slice-builder use. The \`design\` specialist (v8.14+) is **main-context** and emits no slim summary — its Phase 7 sign-off drives the resume decision directly, so it has no place in this enum. Research dispatches (\`repo-research\`, \`learnings-research\`) always emit \`continue\` (no hard-gate authority). The full enum semantics:
...
- any \`=no\` in \`AC verified\` outside \`ceremonyMode: inline\` blocks finalize; orchestrator routes through the reviewer / slice-builder auto-fix loop per the matrix below.
...
- Continuing under \`/cc\` from a stopped flow → specialist Phase 0 reads \`triage.assumptions\` from disk and **does not re-prompt**.
- \`flow-state.json\` already has \`triage.assumptions\` populated (mid-flight continuation **or** pre-v8.21 flows where the legacy preflight step captured the list) → read as ground truth; specialist Phase 0 short-circuits the ask.
...
- **Build-failure routing.** Routed through the v8.61 always-auto failure matrix — see \`runbooks/always-auto-failure-handling.md\` (build failure → \`slice-builder\` \`fix-only\` auto-fix loop, cap 3).
...
- **Reviewer-critical / required-no-fix routing.** Routed through the v8.61 always-auto failure matrix — see \`runbooks/always-auto-failure-handling.md\` (reviewer block / required-no-fix → \`slice-builder\` \`fix-only\` auto-fix loop, cap 3; the v8.13 hard cap of 5 review/fix iterations still applies as the outer bound).
...
The ship stage uses parallel fan-out (release reviewer + adversarial reviewer in strict + security-reviewer when \`security_flag\`), then a structured user ask for finalization mode (merge / open-PR / push-only / discard-local / no-vcs). \`Cancel\` is NEVER an option in the ship-gate ask — the user invokes \`/cc-cancel\` out-of-band if they want to abandon. The ship-gate ask is the ONLY user-facing structured ask left on the always-auto path; every other gate transitioned away from approval pickers in v8.61.
...
**Per-criterion verified gate (precondition, shipped in v8.48; v8.61 always-auto routing).** The orchestrator MUST parse \`AC verified:\` from both the latest slice-builder and reviewer slim summaries before running finalize. When \`ceremonyMode != "inline"\` AND any AC is \`=no\` (or either summary is missing the line), refuse finalize and route through the reviewer auto-fix loop (cap = 3 iterations). After the 3rd failed iteration, surface the stop-and-report status block. No \`accept-unverified-and-finalize\` escape hatch; reviewer's verdict overrides slice-builder's self-attestation. Full procedure lives in \`runbooks/finalize.md > ## Per-criterion verified gate\`.
...
- **Per-criterion verified gate runs before finalize (v8.48+).** Parse the \`AC verified:\` line from the latest slice-builder and reviewer slim summaries; if any AC is \`=no\` outside \`ceremonyMode: inline\`, route through the reviewer auto-fix loop per the failure matrix.
...
\`reviewer\` is multi-mode (\`code\` / \`text-review\` / \`integration\` / \`release\` / \`adversarial\`). \`security-reviewer\` is separate; invoke it when the diff or task touches authn / authz / secrets / supply chain / data exposure. \`triage\` is the v8.61 lightweight router moved to a sub-agent; it runs exactly once per fresh \`/cc <task>\` (research-mode and extend-mode forks skip it).
