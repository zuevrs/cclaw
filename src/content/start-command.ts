...
// `### Detect — patch-mode fork`, `### Detect — research-mode fork`,
...
| \`/cc\` argument starts with \`patch \` (post-ship micro-edit fork) | \`patch-mode.md\` |
| plan-critic (any rubricMode) / critic gates fire | \`critic-steps.md\` |
| \`/cc\` argument starts with \`extend \` (parent-context inheritance) | \`extend-mode.md\` |
...
The persisted triage shape, the audit log schema, the critic-stage insertion rule, follow-up-bug detection, prior-context consumption (extend-mode), and prior-learnings consumption (architect owns the lookup) all live in \`runbooks/triage-gate.md\`. Open that runbook every fresh triage dispatch.

Always-auto: \`triage.runMode\` is \`"auto"\` on every non-inline path and \`null\` on inline. There is no step-mode opt-in or surface anywhere in the orchestrator.
...
| \`plan\` *(sub-step)* | \`plan-critic\` *(gated; \`rubricMode: generic\` — default)* | ceremonyMode=strict + complexity≠trivial + problemType≠refines + AC count ≥ 2 | \`pre-impl-review\` (envelope: \`rubricMode: "generic"\`) | — (self-contained) | \`agents/plan-critic.md\` + \`runbooks/critic-steps.md\` ("Pre-implementation pass — generic mode") | no (gate forbids \`inline\`) |
| \`plan\` *(sub-step)* | \`plan-critic\` *(gated; \`rubricMode: design\`)* | (\`triage.designSurface == true\` OR \`triage.surfaces\` ∩ {ui, design, frontend, ux} ≠ ∅) + ceremonyMode ∈ {soft, strict} + plan.md exists | \`pre-impl-review\` (envelope: \`rubricMode: "design"\`) | — (self-contained) | \`agents/plan-critic.md\` + \`runbooks/critic-steps.md\` ("Pre-implementation pass — design mode") | no (gate forbids \`inline\`) |
| \`plan\` *(sub-step)* | \`plan-critic\` *(gated; \`rubricMode: devex\`)* | (\`triage.devexSurface == true\` OR \`triage.surfaces\` ∩ {cli, library, api} ≠ ∅) + ceremonyMode ∈ {soft, strict} + plan.md exists | \`pre-impl-review\` (envelope: \`rubricMode: "devex"\`) | — (self-contained) | \`agents/plan-critic.md\` + \`runbooks/critic-steps.md\` ("Pre-implementation pass — devex mode") | no (gate forbids \`inline\`) |
...
| \`qa\` | \`qa-runner\` *(gated)* | \`triage.surfaces\` ∩ {ui, web} ≠ ∅ AND \`ceremonyMode != "inline"\` | \`browser-verify\` | qa-and-browser | \`agents/qa-runner.md\` + \`runbooks/qa-stage.md\` | no (gate forbids \`inline\`) |
...
| \`critic\` | \`critic\` | ceremonyMode != \`inline\` | \`gap\` (default, soft + strict-no-trigger) or \`adversarial\` (strict + §8 trigger fires) | — (self-contained) | \`agents/critic.md\` + \`runbooks/critic-steps.md\` ("Post-implementation pass") | no (skipped on \`inline\`) |
...
#### plan-critic (sub-step of \`plan\`; three rubric modes)

\`agents/plan-critic.md\` + \`runbooks/critic-steps.md\` ("Pre-implementation pass — generic / design / devex modes"). One specialist, **three rubric modes** dispatched via the \`rubricMode: "generic" | "design" | "devex"\` envelope fan-out. The orchestrator may dispatch \`plan-critic\` up to three times per slug, sequentially (never parallel); each mode is independently gated; gates + verdict routing + flow-state field shape live in the runbook.

- **\`rubricMode: generic\` (default).** Adversarial structural pass (goal coverage / granularity / dependency accuracy / parallelism feasibility / risk catalog + §2.A Decision-integrity + §2.6.5 Bets-and-exclusions) on gate {ceremonyMode=strict, complexity!=trivial, problemType!=refines, AC count>=2}. Verdicts \`pass\` / \`revise\` / \`cancel\`. flow-state: \`planCriticVerdict\` / \`planCriticIteration\` / \`planCriticDispatchedAt\`.
- **\`rubricMode: design\`.** Seven-dimension design-quality rubric (shared with the reviewer's design-quality axis); below-6 grades → \`PD-N\` rows (severity \`low\`/\`medium\`/\`high\`; accessibility one-tier escalation); appended to plan.md's \`## Plan-design findings\`. Gate: \`triage.designSurface == true\` OR \`triage.surfaces\` ∩ {\`ui\`, \`design\`, \`frontend\`, \`ux\`} ≠ ∅ AND ceremonyMode ∈ {\`soft\`, \`strict\`} AND plan.md exists. Runs after \`generic\` (when generic skipped: directly after architect). Verdicts \`pass\` / \`revise\` / \`block\`. flow-state: \`planDesignVerdict\` / \`planDesignIteration\` / \`planDesignFindingsCount\` / \`planDesignDispatchedAt\` (readers branch on rubricMode, not lastSpecialist).
- **\`rubricMode: devex\`.** Six-dimension DevEx rubric (Getting Started / API ergonomics / Error messages / Docs / Upgrade path / Measurement); \`DX-N\` findings appended to plan.md's \`## Plan-devex findings\`; getting-started one-tier escalation, upgrade-path cap on breaking changes. Gate: \`triage.devexSurface == true\` OR \`triage.surfaces\` ∩ {\`cli\`, \`library\`, \`api\`} ≠ ∅ AND ceremonyMode ∈ {\`soft\`, \`strict\`} AND plan.md exists. Sequential after \`design\` (when design skipped: after \`generic\` or architect). Verdicts \`pass\` / \`revise\` / \`block\`. flow-state: \`planDevexVerdict\` / \`planDevexIteration\` / \`planDevexFindingsCount\` / \`planDevexDispatchedAt\`.
