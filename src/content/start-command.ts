...
| trigger | runbook |
| --- | --- |
| \`/cc\` argument starts with \`research \` (v8.65+ multi-lens; v8.103 lift; v8.112 retired the equivalent flag form) | \`research-mode.md\` |
...
| compound capture is the 5th (v8.112 retired the manual stage-shortcut router that exposed this) | \`compound-refresh.md\` |
...
| \`/cc\` two-reviewer per-task loop (T3-3, obra pattern; v8.13; v8.112 retired the namespace router stage shortcuts) | \`detect-matrix.md\` |
...
1. Build the dispatch envelope per \`runbooks/dispatch-envelope.md\`. The envelope MUST include: \`Task:\` (raw \`/cc\` argument after Detect-hop prefix stripping), \`Project root:\` (for the no-git check), \`Active flow state: null\` (the Detect matrix already confirmed no active flow; extend-mode dispatches here carry the resolved \`parentContext\` instead), and \`Prior research:\` (when \`flow-state.json > priorResearch\` is set; otherwise omit). v8.112 retired the \`Override flags:\` envelope line entirely: ceremonyMode is the triage heuristic's decision, and the v8.61-era run-mode back-compat toggles were noise.
...
5. Append one line to \`.cclaw/state/triage-audit.jsonl\` (\`autoExecuted: true\`; v8.112 retired the \`userOverrode\` branch — the per-flow ceremony override flags it gated are gone).
...
Always-auto: \`triage.runMode\` is \`"auto"\` on every non-inline path and \`null\` on inline (step mode retired in v8.61; v8.112 retired the back-compat run-mode toggles — there is no longer a step-mode opt-in or surface anywhere in the orchestrator).

The triage decision is **immutable** for the lifetime of the flow. To change \`complexity\` / \`ceremonyMode\` / \`path\` / \`mode\`, the user invokes \`/cc-cancel\` and starts fresh. v8.112 retired the user-facing per-flow ceremony override flags; the triage heuristic is the sole source of truth. To influence the heuristic, pin via the task wording itself ("just a typo", "small refactor", "auth migration") — the heuristic reads those signals deterministically.
...
\`agents/critic.md\` + \`runbooks/critic-steps.md\` ("Post-implementation pass"). Skipped on \`ceremonyMode: inline\`. Verdict \`pass\` / \`iterate\` / \`block-ship\`; block-ship → stop-and-report. **Cross-model second opinion (v8.74 trigger; v8.112 santa-loop convergence).** Stamp \`crossModelCritic: true\` when ANY of: \`triage.securityFlag == true\`; any D-N marked \`Reversibility: one-way\`; OR (keyword fallback when plan.md has no \`## Decisions\` section) Blast-radius cites data loss / migration / public-API / payment / auth / cryptography; OR \`/cc <task> --critic-cross-model\`. v8.112 promoted one-shot → **convergence loop** (gstack \`/codex\` MCP pattern + santa-loop): Critic A + Critic B (Codex / Gemini MCP) cold in parallel; **BOTH must \`verdict: pass\`**; divergence ⇒ \`mode: fix-only\` builder re-dispatch from merged findings, then critics re-run afresh; **cap 3 rounds**, after which non-convergence is \`block-ship\` (user reviews → \`/cc-cancel\` or \`/cc patch\`). Counter persists on \`flow-state.json > criticConvergenceRound\` (0 = fallback; 1-3 = active). Graceful fallback writes \`Cross-model unavailable: skipped.\` and degrades to single-critic gating. Full procedure (X-F-N numbering, round shape, fix-only merge rules, four-technique scaffold) lives in the runbook.
...
After ship, dispatch the learnings sub-agent on any compound signal (non-trivial decision recorded by architect; review needed ≥3 iterations; reviewer \`security\` axis flagged) — writes \`flows/<slug>/learnings.md\` + appends \`.cclaw/knowledge.jsonl\`; otherwise honour the **learnings hard-stop** (T1-13; ship runbook §7a) via stop-and-report. (v8.112 retired the manual learnings-capture override; the 4-condition heuristic is now the sole gate.) \`runCompoundAndShip\` then runs two outcome-loop capture paths (**revert** scan stamps prior slugs \`outcome_signal: "reverted"\`; **manual-fix** 24h-window fix-commit scan over the current slug's \`touchSurface\` stamps \`outcome_signal: "manual-fix"\`); the third path (**follow-up-bug**) fires at Triage. Every 5th capture MAY trigger the compound-refresh sub-step (T2-4 everyinc) — full procedure in \`runbooks/compound-refresh.md\`.
