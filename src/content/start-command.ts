...
// `#### Phase 2 — parallel lens dispatch`, `#### critic`, `#### plan-design`,
// `#### plan-devex`, `### Dispatch envelope`, the four-row invocation matrix
...
| plan-critic / plan-design / plan-devex / critic gates fire | \`critic-steps.md\` |
...
| \`plan\` *(sub-step, v8.51; widened v8.54)* | \`plan-critic\` *(gated)* | ceremonyMode=strict + complexity≠trivial + problemType≠refines + AC count ≥ 2 | \`pre-impl-review\` | — (self-contained) | \`agents/plan-critic.md\` + \`runbooks/critic-steps.md\` ("Pre-implementation pass") | no (gate forbids \`inline\`) |
| \`plan\` *(sub-step, v8.75)* | \`plan-design\` *(gated)* | (\`triage.designSurface == true\` OR \`triage.surfaces\` ∩ {ui, design, frontend, ux} ≠ ∅) + ceremonyMode ∈ {soft, strict} + plan.md exists | \`pre-impl-design\` | — (self-contained) | \`agents/plan-design.md\` + \`runbooks/critic-steps.md\` ("Plan-design pass") | no (gate forbids \`inline\`) |
| \`plan\` *(sub-step, v8.82)* | \`plan-devex\` *(gated)* | (\`triage.devexSurface == true\` OR \`triage.surfaces\` ∩ {cli, library, api} ≠ ∅) + ceremonyMode ∈ {soft, strict} + plan.md exists | \`pre-impl-devex\` | — (self-contained) | \`agents/plan-devex.md\` + \`runbooks/critic-steps.md\` ("Plan-devex pass") | no (gate forbids \`inline\`) |
...
\`agents/plan-critic.md\` + \`runbooks/critic-steps.md\` ("Pre-implementation pass"). Walks five dimensions (§1-§5) + §6 pre-commitment predictions; verdicts \`pass\` / \`revise\` / \`cancel\`; 1 revise loop max.

#### plan-design (v8.75+, sub-step of \`plan\`)

\`agents/plan-design.md\` + \`runbooks/critic-steps.md\` ("Plan-design pass"). Gates on \`triage.designSurface == true\` OR \`triage.surfaces\` ∩ {\`ui\`, \`design\`, \`frontend\`, \`ux\`} ≠ ∅ AND \`triage.ceremonyMode\` ∈ {\`soft\`, \`strict\`} AND plan.md exists. **Dispatch ordering:** runs after plan-critic when plan-critic's gate fires (architect → plan-critic → plan-design → builder); when plan-critic is structurally skipped plan-design dispatches directly after architect. Walks the seven-dimension design-quality rubric (shared with the v8.70 reviewer's design-quality axis); findings ladder maps below-6 grades to \`PD-N\` rows with severity tiers (\`low\` / \`medium\` / \`high\`). **Verdict routing.** \`pass\` → orchestrator advances to builder dispatch (or qa-runner if the surface gate also fires post-build). \`revise\` iter 0 → bounce to architect with the open \`PD-N\` rows prepended as the §8 hand-off; orchestrator re-dispatches plan-design (iter 1). \`revise\` iter 1 → stop-and-report status block. \`block\` (any iteration) → stop-and-report status block immediately. flow-state patches: \`planDesignVerdict\` / \`planDesignIteration\` / \`planDesignFindingsCount\` / \`planDesignDispatchedAt\`. Combined revise hand-off when plan-critic AND plan-design both flag in same iteration (architect resolves both in one rewrite). Full procedure lives in the runbook.

#### plan-devex (v8.82+, sub-step of \`plan\`)

\`agents/plan-devex.md\` + \`runbooks/critic-steps.md\` ("Plan-devex pass"). Gates on \`triage.devexSurface == true\` OR \`triage.surfaces\` ∩ {\`cli\`, \`library\`, \`api\`} ≠ ∅ AND \`triage.ceremonyMode\` ∈ {\`soft\`, \`strict\`} AND plan.md exists. **Dispatch ordering:** sequential after plan-design (architect → plan-critic? → plan-design? → plan-devex? → builder); when plan-design is gated off but devex-surface gate fires, runs directly after plan-critic OR architect. Walks six DevEx dimensions; \`DX-N\` findings ledger appended to plan.md. **Verdict routing.** \`pass\` → orchestrator advances to builder dispatch. \`revise\` iter 0 → bounce to architect with the open \`DX-N\` rows prepended as the §8 hand-off; orchestrator re-dispatches plan-devex (iter 1). \`revise\` iter 1 → stop-and-report status block. \`block\` (any iteration) → stop-and-report status block immediately. flow-state patches: \`planDevexVerdict\` / \`planDevexIteration\` / \`planDevexFindingsCount\` / \`planDevexDispatchedAt\`. Combined revise hand-off applies when 2+ pre-impl lenses fire. Full procedure lives in the runbook.
