...
3a. **Populating the \`## Key assumptions to validate\` section (v8.80).** 2-5 bullets naming **bets** the research rests on — beliefs about user demand, market state, technology behaviour, performance characteristics, or downstream system capability that the lenses absorbed as load-bearing premises rather than as findings. Each bullet pairs the bet with a validation method (benchmark, user research, log query, A/B test, prior-art scan) and a status (\`unvalidated\` on first authoring; \`validated\` / \`invalidated\` once evidence lands). Format: \`- **<assumption>** — Validate by: <method>. Status: <unvalidated | validated | invalidated>\`. Distinct from \`## Framings considered\` (those are alternative shapes of the question; this section is the implicit beliefs the framings rely on). The follow-up \`/cc <task>\` flow's architect Bootstrap copies the bullets verbatim into \`plan.md > ## Key assumptions to validate\` so the bets carry forward.
...
- The reviewer applies the **eleven-axis** check (correctness / test-quality / readability / architecture / complexity-budget / security / perf / qa-evidence / edit-discipline / nfr-compliance / design-quality — see reviewer.md for the per-axis checklist). v8.62 absorbed \`security-reviewer\` into the \`security\` axis (full threat-model + sensitive-change protocol). v8.70 added the gated \`design-quality\` axis (see "Auto-activate design-quality axis" below).
...
- **review-discipline** — wraps every reviewer invocation; Findings + eleven-axis pass + convergence detector.
