...
The opening step of every \`/cc\` dispatch. The triage gate sets \`complexity\` (\`small-medium\` vs \`large-risky\`), \`path\` (inline / small-medium / large-risky), and \`ceremonyMode\` (\`strict\` vs \`soft\`). Triage is immutable mid-flight. The \`ceremonyMode\` field was named \`acMode\` pre-v8.56; legacy state files are hoisted on read.
